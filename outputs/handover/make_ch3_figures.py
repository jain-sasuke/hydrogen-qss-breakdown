...
def main():
    ...
    plt.close(fig)
    print(f"fig3.1  tau_QSS={tau[0]:.4e}  tau_relax={tau[1]:.4e}  "
    ...
    def b1_cre(i, j):
        ...
        the ionisation balance has settled. This is NOT the transient b_1(t)
        realised after a temperature step -- distinguishing the two is the
        subject of the chapter, and the figures must not blur them."""
    ...
    ax.semilogx(bb, R, color="#c0392b", lw=1.6, zorder=3)
    ...
    plt.close(fig)
    print(f"fig3.4  max|f3-f4|={abs(sens[k]):.4f} at b_1={bb[k]:.3e}; "
    ...
    XR = 0.68                       # right edge of the level column
    XA = 0.74                       # x of the annotation column

    ax.axhline(ROW["cont"], color="0.15", lw=1.4, xmin=0.10, xmax=0.66)
    ...
    ax.text(XA, ROW[3], r"$n \leq 8$: $\ell$-resolved", fontsize=7.5,
            va="center", color="0.3")

    ax.text(0.36, ROW["gap"], r"$\vdots$", fontsize=9, ha="center",
            color="0.45")
    ax.text(XA, ROW["gap"], r"$n = 5$--$8$: $\ell$-resolved, not drawn",
            fontsize=7, va="center", color="0.45")

    for n in (9, 12, 15):
        ax.plot([0.24, 0.48], [ROW[n]] * 2, color="0.45", lw=1.2)
        ax.text(0.50, ROW[n], rf"$n={n}$", fontsize=7, va="center",
                color="0.45")
    ax.text(XA, ROW[12], r"$n = 9$--$15$: bundled", fontsize=7.5,
            va="center", color="0.45")

    # proton l-mixing within n = 2 and n = 3
    for n in (2, 3):
        for x0, x1 in zip(xs[n][:-1], xs[n][1:]):
            ax.add_patch(FancyArrowPatch((x0 + 0.05, ROW[n]),
                                         (x1 - 0.05, ROW[n]),
                                         arrowstyle="<|-|>", mutation_scale=7,
                                         lw=1.0, color="#e67e22"))
    ax.text(0.34, ROW[2] - 0.28, r"proton $\ell$-mixing", fontsize=7,
    ...
    ax.text(0.455, (ROW[1] + ROW[2]) / 2, r"$A_{2p\to1s}$", fontsize=7,
            color="0.35")
    # collisional 2s -> 1s only: no E1 path, and no two-photon rate in L
    ax.add_patch(FancyArrowPatch((0.26, ROW[2] - 0.08), (0.32, ROW[1] + 0.10),
                                 arrowstyle="-|>", mutation_scale=7, lw=0.9,
                                 color="#2e7d32", ls=(0, (4, 2)),
                                 connectionstyle="arc3,rad=0.2"))
    ax.text(0.235, (ROW[1] + ROW[2]) / 2,
            "collisional only:" "\n" r"no E1 $2s\rightarrow1s$",
            fontsize=6.5, color="#2e7d32", ha="right", va="center")

    # the partition
    ax.add_patch(Rectangle((0.12, ROW[1] - 0.34), 0.56, 0.60, fill=False,
                           ec="#c0392b", lw=1.1, ls="--"))
    ax.text(0.13, ROW[1] - 0.52, r"slow: $\mathcal{S} = \{1s\}$",
            fontsize=8, color="#c0392b", va="top")
    ax.add_patch(Rectangle((0.12, ROW[2] - 0.40), 0.56,
                           ROW[15] - ROW[2] + 0.72, fill=False,
                           ec="#1f4e79", lw=1.1, ls="--"))
    ax.text(0.13, ROW[15] + 0.44,
            r"fast: $\mathcal{F}$, 42 excited levels", fontsize=8,
            color="#1f4e79", va="bottom")

    # a and c: network responses, drawn well outside the level column
    ax.add_patch(FancyArrowPatch((0.08, ROW[1]), (0.08, ROW[4]),
                                 arrowstyle="-|>", mutation_scale=12,
                                 lw=2.6, color="#2e7d32", alpha=0.5))
    ax.text(0.055, (ROW[1] + ROW[4]) / 2,
            "ground-fed" "\n" r"response $\mathbf{a}$",
            fontsize=7.5, color="#2e7d32", ha="right", va="center")
    ax.add_patch(FancyArrowPatch((0.71, ROW["cont"]), (0.71, ROW[9]),
                                 arrowstyle="-|>", mutation_scale=12,
                                 lw=2.6, color="#8e44ad", alpha=0.5))
    ax.text(0.695, (ROW["cont"] + ROW[9]) / 2,
            "recombination-fed" "\n" r"response $\mathbf{c}$",
            fontsize=7.5, color="#8e44ad", ha="right", va="center")

    # the explanation of a and c belongs in the caption, not the drawing;
    # only the "not to scale" warning has to be on the figure itself
