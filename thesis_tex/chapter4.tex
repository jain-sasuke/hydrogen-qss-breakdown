...
%     convergence result carried in Section~\ref{sec:convergence} still
%     rests on working notes (ADDENDUM D.8) with no producing script; that
%     gap is recorded in Appendix C and in the ladder table, not hidden. Its \todo asking for provenance on the
...
what the terminal shell loses is a way out and not a way in; an earlier draft
said it had ``nothing above it to cascade out to'', which names the wrong
process. The loss removed is not small. Continuing the $12\to13$, $13\to14$
...
$r_0(15)$ agrees (Section~\ref{sec:fujimoto}). Comparing each shell's population when it is terminal against its
...
faster than $\tauslow$, and the reason can be stated exactly rather than
asserted. Once the manifold is quasi-steady the excited vector is a function of
$u$ alone, so
...
$\tauslow = \SI{67.2}{\second}$ at the cold corner $[0,0]$.
...
That reading is no longer supportable, for six reasons, and the honest status
...
resolves $\ell$ below $n=9$. That difference is real, and an earlier draft of
this section treated it as the explanation. It is not. Imposing the source's own
...
disagreement, and this thesis does not explain it. Three candidate causes have
each been tested and each fails to account for a factor of $8$: truncation
(\SI{1.2}{\percent}), Lyman trapping (which fixes $p=2$ only by driving $r_0(2)$
above Saha), and the $\ell$~closure (\SI{0.42}{\percent} at $p=3$, and in the
wrong direction at $p=2$; \texttt{verify\_fujimoto\_bundle.py}). That last test
imposes the source's own closure rather than approximating it, and it carries a
severity check: bundling an operator whose $\ell$~distribution is deliberately
...
Chapter~\ref{ch:results} uses untouched, and an earlier draft said it did.
...
(\texttt{validation/reservoir\_vs\_adas/}). One thing about the read matters
here. ADAS96 tabulates seven temperatures between $1$ and
\SI{10}{\electronvolt}, and \texttt{diagnose\_gate\_d.py}, the script that
first made this comparison, interpolates the coefficient linearly in value
between them (its function \texttt{adas\_at}, lines 114 to 124); across the
$1$ to \SI{1.5}{\electronvolt} interval, where SCD rises by nearly two orders
of magnitude, that overstates the table by up to a factor $6.65$. An earlier
draft quoted the numbers from that read, $323$ of $400$, a range of $0.130$ to
$0.931$ and a median of $0.696$, and reported a deficit that deepened toward
low temperature. Read logarithmically in both axes, which is the read the
table's spacing requires, the low-temperature tail disappears: the row median
is $0.72$ at \SI{1}{\electronvolt} and $0.90$ by \SI{1.2}{\electronvolt}. The
diagnosis script is reported and left in place; every ADAS number in this
section now comes from the logarithmic read.
...
\SI{3.3}{\percent} at the benchmark, \SI{1.8}{\percent} at the grid median, and
within \SI{11}{\percent} at every point. An
...
$\eta = 1.004$ at $[4,7]$. An earlier draft reported the gap closing steeply
with temperature, from a median of $0.302$ over $1$ to \SI{1.5}{\electronvolt}
to $0.817$ over $5$ to \SI{10}{\electronvolt}, and read that shape as the
signature of truncation; the steep part was the linear read of the table, and
under the logarithmic read the deficit is between $0.72$ and $0.94$ at every
...
shape no longer singles that cause out. The share of ground-fed ionisation
flux carried by shells $n \ge 12$, and its correlation with the deficit, were
computed by the diagnosis script against the linear read and printed rather
than stamped; they are not carried forward. The test that would settle the
...
One condition attaches. ADAS96 tabulates seven temperatures between $1$ and
\SI{10}{\electronvolt}, and its coefficients have to be read logarithmically
in both axes. Read linearly in value, as \texttt{diagnose\_gate\_d.py} reads
them, SCD is overstated by up to a factor $6.65$ across the $1$ to
\SI{1.5}{\electronvolt} interval and the refuter would have appeared at $9$ of
the $264$ steps. That defect in the diagnosis script is reported here and left
in place; every number in this paragraph comes from the logarithmic read.
...
separately gives agreement with ADAS within a factor two at all $400$ points
for SCD, the model sitting $0$ to \SI{30}{\percent} low with a median ratio
of $0.83$, a deficit consistent with the $n_{\max}$ truncation but not
uniquely so, and at all $400$ for ACD with a median ratio of $0.98$.
...
\SI{3.3}{\percent} at the benchmark and to a median \SI{1.8}{\percent} across the
grid. The failure does not touch the structural results
...
\textbf{The literal survived in two further files, and has now been
removed.} After the production filter was corrected it remained at
\texttt{src/rates/solve\_cr.py}:269 and \texttt{src/rates/check\_mz.py}:10. Both
were confirmed to be imported by nothing, so no number in this thesis ever passed
through them; that was luck rather than design, and it is recorded here because a
commit message claiming the filter had been removed is part of the same history.
Both were corrected on 16 September 2026, once the absence of any consumer had
been established, and the literal no longer appears anywhere in \texttt{src/}.
The correction changes no number in this thesis: at the benchmark the two filters
return the same $\tauslow$ to every digit, and they differ only where the slow
mode is slower than $1\,\si{\per\second}$, which is the cold corner the filter was
silently truncating in the first place.
...
unfixed failure of this kind. It is also incomplete, and more so than an
earlier draft said. Its test for a write reads the file mode from the second
...
which is the number the earlier draft gave; in the tree this thesis is built
...
Four of the checks graded severe are in that condition, which is a statement
about this thesis's record-keeping rather than about the model. Of the eighteen
internal checks, eleven are severe, one is documented but not re-executable,
four are tautological and two are weak; of the five external comparisons, two
...
One energy ladder on both sides & severe & notes & Saha ratio $0.999997$ for all $43$ states \\
...
Truncation convergence of $\ffrac{3}-\ffrac{4}$ & documented, not re-executable & notes & increments geometric at ratio $0.75$; extrapolated \SI{+0.9}{\percent} at the benchmark, \SI{-1.4}{\percent} at the cold corner. Tests one quantity, not the state space; no script regenerates it \\
...
and finds it converged to between $+0.9$ and \SI{-1.4}{\percent} at the tested
points, from working notes that no script regenerates; convergence of the
reservoir quantities $u^{\mathrm{CRE}}$, $\Delta\ln u$ and $\tauslow$ under
truncation is not established (Appendix~\ref{app:numerics}). What is measured
...
fails at every point on the grid, and Section~\ref{sec:gate_d} demonstrates
why: it compares a coefficient that mixes both channels against a table that
separates them. Rebuilt on the ground-fed channel, SCD agrees within a factor
two at all $400$ points, $0$ to \SI{30}{\percent} low with median ratio $0.83$,
and ACD agrees at $400$ of $400$; the model's total effective ionisation
coefficient remains a quantity ADAS does not tabulate.
...
bundled shells, which is assumed rather than measured. The test of that
assumption has been run (Section~\ref{sec:bundling_check}): mixing clears
radiative loss by three orders of magnitude everywhere and total collisional
escape by a factor $2.85$ to $26$ with the model's frozen cutoff, but not at
the two highest densities below \SI{3}{\electronvolt} once the cutoff is
evaluated locally. The assumption is licensed over most of the grid and rests
on the $\ell$-blindness of $n$-changing collisions at the rest.
