...
%     match. sec:nmax states the n_max convergence check has never been run;
%     it has now been run (ADDENDUM D.8) and only the SEPARATE bundled-shell
%     l-distribution check remains unrun. Fix that sentence.
%
...
Three quantities currently printed by this project's own scripts as validation
are tautological, and Sections~\ref{sec:conservation_severity}
and~\ref{sec:gates_abc} say so with the measurements that establish it. Naming
...
measured residual, Eq.~\eqref{eq:conservation_measured}, as
$3.61\times10^{-11}$. The claim attached to it there is that ``an error in a
...
\begin{table}[tbp]
  ...
  \begin{tabular}{lrl}
    ...
    $C(1s\to2p)$ multiplied by ten      & $6.9\times10^{5}$    & $2.238\times10^{-12}$ \\
    all de-excitation deleted           & $1.9\times10^{11}$   & $2.171\times10^{-12}$ \\
    excitation array transposed         & $2.7\times10^{11}$   & $1.837\times10^{-12}$ \\
    $A$ halved, $\gamma$ left alone     & $3.1\times10^{8}$    & $3.63\times10^{1}$ \textbf{caught} \\
    \bottomrule
  \end{tabular}
\end{table}
...
The one fault that was caught is the one that breaks the construction. The
radiative loss rates $\gamma_p = \sum_{q<p} A_{pq}$ enter the diagonal, while
the individual $A_{pq}$ enter the off-diagonals; halving one without the other
makes the two inconsistent, and the residual jumps by thirteen orders of
magnitude to $36.3$. \textbf{The conservation check tests one thing: that $A$
and $\gamma$ were built from the same table.} It is worth running, it raises
rather than warns, and it is exactly that narrow.
...
Gate~A takes every excitation coefficient $K_{\rm exc}(i\to j)$ in the dataset,
predicts the reverse coefficient from the Boltzmann relation
\begin{equation}
  K_{\rm deexc}(j\to i)
  ...
  \label{eq:gateA}
\end{equation}
and compares the prediction against the stored reverse coefficient at three
temperatures spanning the grid. Here $g_i$ is the statistical weight of level
$i$ and $E_j-E_i$ its excitation energy in \si{\electronvolt}. The gate passes
...
levels whose Boltzmann factors span $\mathrm{e}^{13.6\,\mathrm{eV}/k_B\Te}$
down to $\mathrm{e}^{0.06\,\mathrm{eV}/k_B\Te}$. A ratio that is
state-independent across thirteen decades of exponential cannot be produced by
two energy ladders that differ; the common residual is the eight-figure
rounding of the stored state table. \textbf{Grade: severe.} Two energy tables
would have shown up as a drift across the ladder, and there is none.

The same audit gives the collisional side over $2457$ excitation-pair
comparisons spanning three temperatures: maximum deviation $7.31\times10^{-9}$,
median $7.5\times10^{-10}$, again consistent with the state table's rounding.
% 819 pairs x 3 temperatures = 2457. ADDENDUM D.6 writes "2457 pairs x 3
% temperatures", which would be 7371 comparisons. The phrasing above is true
% under either reading; resolve the count before submission.
...
line 239, stored, printed at line 261, and never consulted). Measured, the
excluded column is the informative one: the approach fraction runs
$0.0123$ to $0.0155$, so at $\nel = 10^{15}\,\si{\per\cubic\centi\metre}$, the
top of the grid, populations reach about \SI{1.5}{\percent} of Saha.

Local thermodynamic equilibrium (LTE) is therefore never approached anywhere on
this grid. That is the correct physics for an optically thin plasma whose ion
population is an externally imposed reservoir rather than a Saha partner, and
Chapter~\ref{ch:results} depends on it: it is why the two supply channels of
Section~\ref{sec:R_depends_on_b} never merge, and why $\ffrac{3}-\ffrac{4}$ does
not vanish at high density.
...
fraction reported rather than suppressed.
...
eigenvalues, and forms $\Msep = \tauqss/\taurel = |\lambda_1|/|\lambda_0|$. It
...
corner; $\tauqss$ spans \SI{75.4}{\nano\second} to \SI{67.2}{\second} and
$\taurel$ spans \SI{0.87}{\nano\second} to \SI{38.9}{\nano\second}
(\texttt{thesis\_ready.md} A1). The gate asks for $\Msep>1$. The smallest value
anywhere is $86.8$.

\textbf{Grade: tautological as posed, and the real content is elsewhere.} A
$43$-state stiff system with radiative rates of order $10^{8}\,\si{\per\second}$
and an ionisation rate of order $10^{4}\,\si{\per\second}$ cannot have
$\Msep \le 1$; the gate cannot fail on any matrix this construction produces.
...
Two properties of these numbers must travel with them, and both are
established in Chapter~\ref{ch:theory} rather than here. $\tauqss$ is
boundary-dependent, which Section~\ref{sec:attacks} quantifies. And
$\lambda_0 = 2.286\,K_{\rm ion}(1s)\,\nel$ is the collisional--radiative
ionisation rate of ground-state hydrogen, not an equilibration rate, so
$\tauqss$ is the lifetime of the reservoir and not the time for anything to come
to equilibrium.
...
not, and that asymmetry is a defect rather than a subtlety.
...
that the excited populations split exactly into a ground-fed and a
recombination-fed part,
$\nF = \bm{a}\,\ngs + \bm{c}\,\nion$, with $\bm{a}$ and $\bm{c}$ independent of
the reservoir densities. If that split is not exact, the ground-fed fractions
$\ffrac{p}$ are not well defined and the closed form for the diagnostic error
does not exist.
...
relative residual of the sum against the joint solve. Over all $784$
grid-point/direction pairs the largest residual is $3.075\times10^{-14}$
...
that would most easily go unnoticed in a two-shell observable, changes the
residual and trips the gate. It is one of only two checks in the project known
to catch that swap.
...
\texttt{src/validation/preflight.py} pins the SHA-256 hashes of the two
canonical inputs and compares them byte for byte before any verification script
...
\end{tabular}
\end{center}
Both match the pinned baselines at \texttt{preflight.py}:37--38 exactly. Every
number in this thesis that traces to $\Lmat$ traces to that first hash.
...
rebuilds every plotted quantity from $\Lmat$ and $\Svec$ and compares against
...
It is not. Given $\bm{a},\bm{c}\ge0$, which
Section~\ref{sec:R_depends_on_b} proves from the M-matrix structure of
$-\Lmat_{FF}$, the bound is a theorem: it cannot fail except on an arithmetic
bug. Fed corrupted input it still passes. Swapping shells $3\leftrightarrow4$
leaves both $|\Delta|$ and the logarithmic ratio unchanged; swapping
$c_3\leftrightarrow c_4$ likewise (\texttt{findings\_10} ADDENDUM~B.3).
...
The relevant quantity is the condition number of $\Lmat_{FF}$, the factor by
which a relative perturbation of the matrix can be amplified in the solution.
Computed directly from the canonical $\Lmat$ at all $400$ grid points:
minimum $1.4821\times10^{3}$ at $[0,0]$ ($\Te = \SI{1.000}{\electronvolt}$),
maximum $1.7436\times10^{5}$ at $[33,7]$ ($\Te = \SI{4.715}{\electronvolt}$),
median $2.1044\times10^{4}$. In double precision, which carries about sixteen
...
\textbf{[UNVERIFIED: no committed script produces the grid-wide condition
number.]} The values above are from \texttt{findings\_10} ADDENDUM~C.1
(10 September 2026), which computed them directly from the canonical matrix and
confirmed the range asserted at \texttt{chapter3.tex}:719--721 to
\SI{0.1}{\percent} and \SI{0.2}{\percent}. \texttt{np.linalg.cond} appears at one
site in \texttt{src/}, inside \texttt{verify\_fujimoto\_table41.py}:334, and
covers four point-variants at $\Te = \SI{10}{\electronvolt}$ only. The number is
correct and not yet reproducible from the repository.
\todo{Commit the producing script for the grid-wide $\kappa(\Lmat_{FF})$, then
delete this bracket.}

Three further measurements bound the arithmetic. The eigenvalue condition
numbers of the two eigenvalues the thesis uses are of order unity
($1.71$ and $1.98$ at the benchmark point, $3.26$ and $2.59$ at the cold
corner), despite $\|\Lmat\|_1 = 2.0\times10^{14}$; the large norm is a
...
$10.7$ at $p=9$ and $6.3$ at $p=10$. The externally measured excess at the
terminal shell, a factor $4.9$ to $6.2$ against published values, falls inside
that internally measured band, so the discrepancy at $n=15$ is accounted for by
truncation rather than by the atomic data. No result in this thesis rests on the
top shell: the observable is built from $n=3$ and $n=4$, both fully resolved in
orbital angular momentum $\ell$ and seven shells below the cut.

\paragraph{What has not been checked.} Above $n=8$ the shells are bundled and
their internal $\ell$-distribution is assumed statistical rather than computed.
A script to test that assumption, \texttt{verify\_bundling\_psm20.py}, exists
and has never been executed. It is reported here as not run rather than omitted.
Two defects must be understood before it is: it synthesises grids silently when
its input files are missing (lines 160--166), which violates the project's own
fail-loudly rule and would produce a confident answer from no data; and if the
$\ell$-mixing rate array is zero for the bundled indices, as
\texttt{compute\_lmix.py}:465 suggests, it will read those zeros and return a
verdict of ``bundling invalid'' that is a statement about missing data rather
than about physics.
\todo{Run \texttt{verify\_bundling\_psm20.py} after fixing the silent grid
fallback, or state the bundled $\ell$-distribution as a scope limitation in
Chapter~\ref{ch:limits}. Not both, and not silence.}
...
The optical depth of $H_\alpha$ across a \SI{10}{\centi\metre} slab is
...
whose product stays in the vessel. Since $\tauqss$ is the lifetime of the
ground-state reservoir, and since the persistence claim of
Section~\ref{sec:persistence} rests entirely on $\tauqss$ exceeding the duration
of the disturbance, the obvious attack is that $\tauqss$ is an artifact of the
open boundary.

\textbf{The falsifier, named first.} At $[0,4]$, the worst point,
$\tauqss/\taudrive \approx 2300$. If closing the system shortened $\tauqss$ by
that factor, the slow clock would fall to the duration of the disturbance and
the persistence would vanish. A factor of $2300$ refutes the claim.
...
\begin{table}[tbp]
  ...
  timescale $\tauqss$ computed with the ion treated as an external reservoir
  ...
  maximum $41.4$: the boundary condition matters only where $\tauqss$ already
...
is large only where $\tauqss$ already exceeds the disturbance duration by about
a thousand, and it is unity everywhere $\tauqss$ approaches $\taudrive$. Where
the boundary matters, the outcome does not depend on it, because the error has
already saturated; where the outcome depends on $\tauqss$, the boundary does not
matter. Recomputing the entire persistence census with closed-system $\tauqss$
...
and treats it as a lower bound on the error averaged over a disturbance of
finite duration, on the argument that the observable decays from the plateau on
$\tauqss$. A bound is only useful if it is not absurdly loose, and a bound
quoted as though it were the answer is a claim about tightness that has to be
measured.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    point & $\epsplat$ & true \SI{100}{\micro\second} average & lower bound & true/bound \\
    \midrule
...
The bound is the answer to better than \SI{0.7}{\percent} at every point
tested, which is a stronger statement than the one the thesis makes and should
replace it.

The reason is structural, and it is the property that a different observable in
this same project does not have. The spectrum of
$\Lmat$ has one slow eigenvalue and then a band, with nothing between: at
$\Lmat[1,4]$ the three slowest eigenvalues are $-4.29$, $-1.60\times10^{8}$ and
$-3.44\times10^{8}\,\si{\per\second}$, a gap of $3.7\times10^{7}$. An observable
built from the excited populations cannot decay faster than $\tauqss$, because
there is no intermediate mode for it to decay on. That was the failure mode
being hunted, and it does occur for the ground-normalised error measure of
Section~\ref{sec:eps_definitions}, whose $1/\mathrm{e}$ time is
\SI{2.58}{\second} against $\tauqss = \SI{67.2}{\second}$ at the cold corner.
...
structural feature in the same shell.
...
$0.7392$ against the tabulated $0.730$. That is a severe test passed, and it is
the same quantity that the deliberately broken variant of
Section~\ref{sec:severe_checks} drives to $104.5$.
...
\begin{enumerate}
\item \textbf{The model's $r_1$ reproduces analytically.} In the coronal limit
$r_1(2)$ follows from the single excitation coefficient
$C(1s\to n{=}2) = \SI{9.2796e-9}{\cubic\centi\metre\per\second}$ and gives
$1.37\times10^{-5}$, against the full $43$-state solve's $1.66\times10^{-5}$.
The solve is doing what the hand calculation says it should.
...
case-B recombination coefficient $\alpha_B \approx
\SI{2.31e-13}{\cubic\centi\metre\per\second}$, which is exactly the $n>15$
truncation of Section~\ref{sec:convergence}; and the ground-state ionisation
coefficient, $\SI{4.864e-9}{\cubic\centi\metre\per\second}$, agrees with the
accepted value of about \SI{5e-9}{\cubic\centi\metre\per\second}\needcite.
...
$C(1s\to n{=}2) = \SI{1.44e-7}{\cubic\centi\metre\per\second}$ at
...
\item \textbf{A single-row density offset reconciles the whole column.} The
table's density rows run at $\lg\nel = 12, 14, 16, 17, 18$, non-uniformly
spaced, which is the standard setup for an off-by-one read. Comparing this model
at $\SI{e13}{\per\cubic\centi\metre}$ instead of $\SI{e12}{\per\cubic\centi\metre}$
gives ratios of $0.817$, $0.888$, $0.836$, $0.819$ and $0.977$ for $p = 2$
through $7$: flat to within \SI{10}{\percent}, against a factor-$50$ spread as
published.
\end{enumerate}

The producing script's own docstring instructs that the table values be
re-checked against the book before anything enters the thesis. That re-check was
never performed, and the number entered the thesis anyway.

\textbf{Status.} The $r_1$ comparison at low $p$ is withdrawn as a statement
about this model and reported as an unresolved external comparison. It is not
evidence that the model is wrong, and it is not evidence that the model is
right. What would settle it is one afternoon with the printed table and the
density-row labels. Until that is done, no conclusion rests on it, and the
$r_0$ agreement above is unaffected because it was read from the same rows.
\todo{Re-read Fujimoto Table 4.1(b) directly, with the $\lg\nel$ row labels
verified against the caption, and record the outcome. This supersedes
\texttt{chapter6.tex} \S\ref{sec:r1_deficit}, which still publishes the
discrepancy as a model failure.}

%-----------------------------------------------------------------------------
\section{Gate D fails}
\label{sec:gate_d}
...
Gate~D forms the model's counterpart by taking the steady-state populations at
each grid point, weighting each level by its ionisation rate coefficient, and
dividing by the total neutral population:
\begin{equation}
  {\rm SCD}_{\rm model}
...
Measured, $\eta$ ranges from $5.73$ to $8346$ and no point on the $400$-point
grid agrees within a factor of two. The disagreement is systematic rather than
scattered: a factor between $9$ and $65$, peaking near
$\Te \approx \SI{2}{\electronvolt}$.

\subsection{The diagnosis}

Two readings are open, and neither has been demonstrated.
...
recombination-fed Rydberg levels of the $\nzero$ channel. Ionisation out of
...
$\none$ alone, and it has not been run.

The second is that the two quantities are not defined on the same system at all.
ADAS coefficients describe a closed atom-plus-ion system; this model holds the
ion as a fixed external reservoir with no forty-fourth state
...
The recombination half does not exist. The ACD table is loaded once, at line
299 of \texttt{validate\_gates.py}, and the variable holding it is never read
again anywhere in the file. Only the ionisation comparison runs, despite a docstring
that describes both.
...
\textbf{No agreement with ADAS is claimed anywhere in this thesis.} Gate~D is
reported as failing, with the two candidate diagnoses stated and neither
demonstrated. It is not repaired, no tolerance is widened, and the
$\eta$ range is not restricted to a subset that would pass.

What this costs is less than it first appears and more than nothing, and the
difference should be stated precisely. It costs the thesis its only comparison of
absolute effective coefficients against an external database, which is a real
loss and an examiner will ask about it. It does not touch the structural results
...
\begin{equation}
  ...
  \label{eq:FUm}
\end{equation}
with $U_m = E_{\min}/k_B\Te$ set by the Debye cutoff. $F(U_m)$ is the Coulomb
logarithm of the interaction: for small $U_m$ it behaves as
$\ln(R_c/R_1)$ and diverges logarithmically.
...
recomputing the spectrum: $\tauqss$ moved \SI{+0.07}{\percent} and $\taurel$
...
equilibrated on that timescale either way. \textbf{Timescale separation protects
the slow eigenvalues from errors in the fast rates}, which is the same structure
that Chapter~\ref{ch:theory} exploits to eliminate the fast variables in the
first place, seen from the direction of error propagation.
...
matrix with a density-consistent Debye cutoff rather than a frozen one moves
...
range.
\todo{Correct \texttt{chapter2.tex}:924. The \SI{30}{\percent} figure is not
...
$\lambda_0$, so $\tauqss$, $\taurel$ and $\Msep$ are all wrong together, and all
...
$\tauqss$ \emph{exactly}, which is the ladder shift confirming the mechanism
...
\item The step-error maps were unaffected. $\epsstep$ is bit-identical at all
$400$ points, because it is computed from steady states rather than from the
spectrum, and the benchmark point is bit-identical too.
\end{itemize}
...
One further item belongs here because it is unrepaired.
Section~\ref{sec:ion_recomb} states that at
$\nel = \SI{e12}{\per\cubic\centi\metre}$ three-body recombination supplies
under \SI{10}{\percent} of the feed into any level, and uses that to propose
...
datasets, and \textbf{no such separate test was run}. The sentence in
Chapter~\ref{ch:theory} promises a check this chapter does not deliver.
\todo{Correct \texttt{chapter3.tex}:190--195. Either drop the promise or
construct a test that genuinely separates the two datasets.}
...
and step-error grids reproduce bit-identically, including the values obtained by
numerical integration rather than by linear algebra. The plateau and persistence
...
The timescales reproduce to four digits, $\tauqss = \SI{22.73}{\micro\second}$,
...
identically and the difference would close. It does not happen. $\Delta$ is
nearly flat: $0.979$ at $\SI{e12}{\per\cubic\centi\metre}$, $1.945$ at
...
they would. \textbf{The consequence sharpens Chapter~\ref{ch:results}}: because
the span of $\Delta$ is nearly constant while the operating point sweeps, the
density maximum in the diagnostic error is a position effect rather than a span
effect, and the error does not vanish at the top of the density grid.
...
had. The agreement is close: $\tauqss$ to \SI{0.064}{\percent}, $\taurel$ to
...
Two arrays of timescales, $\Msep$, $\tauqss$ and $\taurel$, are written to the
...
\begin{table}[tbp]
  ...
  fails it. Four of the fourteen entries are severe, one external gate fails,
  and one external comparison is open with an unverified target.}
  \label{tab:ladder}
  \small
...
regeneration, and the conditioning measurements together establish that the
linear algebra does what Chapter~\ref{ch:theory} says it does, to a precision
many orders of magnitude finer than any quantity the thesis reports.
...
hardest agree better than that. Above $n=4$ the atomic data are less certain and
