...
### K20. The ℓ-blindness argument tested at the 121 high-density combinations: the rates are not ℓ-blind, the distribution is statistical by detailed balance among thermalised shells ✅ Verified (22 Sep 2026)

**Why.** K13 left §6.8.3's mechanism untested: at 121 of 2800 (shell, grid) combinations (n = 9..15, nₑ ≥ 3.7e14, Tₑ ≤ 2.95 eV, 34 grid points) the locally evaluated ℓ-mixing loses to collisional loss, and the chapter said the statistical ℓ-distribution was "plausible" there because adjacent n-changing collisions are "close to ℓ-blind at high n". The repository holds the data to test it: the CCC tables resolve ℓ on both sides up to n_f = 10, including all 90 (9,ℓ)→(10,ℓ′) pairs, which `assemble_K_exc.py` only ever uses summed over ℓ_f.

**`verify_l_blindness.py --write`** → `validation/l_blindness/` (log + five csv, sha256 of every input). Gates: G0 CCC rows n_f ≤ 8 equal `K_exc_full`/`K_deexc_full` to the bit and the ℓ_f-sums equal the bundled columns (0); G1 detailed balance on all 1320 × 50 (2e-15); G2 L = R + nₑC (5e-16), the Δn = 0 off-diagonal block of C equals `K_lmix.npy` (1e-16) and `compute_K_lmix(te, 1e14)` reproduces it (0); G3 u_CRE, a₃, a₄, c₃, c₄ vs `molecular_channel.csv` at 400/400 (9e-13); G4 bundled radiative columns of R (1e-11); G5 the 121/34 re-derived from `bundling_total_loss.csv`. Hostile audit (math-auditor + cr-physicist roles): QUOTE WITH CHANGES, all six applied before any number entered the text (radiative share requoted at the 34 points, R3 recast as an identity in θ, Saha departures added, radiative ℓ-dependence estimated, s_max reworded, weighted measures added).

*Predicted (before the run):* upward adjacent spread max/min falls with n but stays > 1.3 at n = 9; downward 1.3–3 at n = 9, 10; feed from a statistical lower shell within 0.5 of statistical at n_f = 9, 10; n = 9 two-neighbour estimate within 0.5; n = 8 without mixing within 0.5; consequence of the bundled radiative columns < 0.5 % at the 34 points. *Found:* **P1–P3, P6 not reproduced.** The upward spread GROWS with n, 1.22 (n = 2) → 2.53–2.75 (n = 9), downward 5.2–5.9 at n = 9 and 5.0–5.7 at n = 10, the yrast sublevel largest in every case; (2ℓ+1)-weighted CV at n = 9, 1 eV: 0.27 up, 0.46 down. Feed from a statistical neighbour: b(ℓ′ = 0) = 0.28, b(yrast) = 1.66 at n_f = 9, 10 (from below); 0.54–1.37 (from above). **Refuter of "close to ℓ-blind" (spread > 3 at n = 9 or 10) APPEARED.** P4, P5 reproduced (n = 9 estimate and n = 8 no-mixing both statistical to ≤ 1e-4 at all 34 points) but not for the predicted reason: with detailed balance exact, p_ℓ/ω_ℓ = [αK_down(ℓ) + βK_up(ℓ)]/[K_down + K_up], so b_ℓ − 1 = (θ − 1)[f_dn(ℓ) − f̄], |f_dn − f̄| ≤ 0.16, an identity (checked to 6e-4); the content is θ, the Boltzmann departure of the neighbour pair, which the model's populations put within 6e-5 of 1 for every bundled pair at the 34 points and within 3e-4 under the s = 0 / s_max radiative rescalings (so not an artefact of the bundled block's assumptions). Stronger: b_Saha(n) − 1 = +3e-6 to +9e-5 for n = 8..15 at the 34 points from the model's own K_ion/α_3BR pair; the high shells are in LTE there, which covers n = 11..15 and the ionisation channel. Complementarity: |θ₉ − 1| > 0.01 at 55/400 points (all nₑ ≤ 2.7e12, 42 inside Tₑ ≥ 2 eV), where the local mixing beats the total loss in every bundled shell by ≥ 341; 0 points fail both. Radiative loss out of 9p is ℓ-dependent (A(9p_tot) = 8.9γ₉) and uncompensated: b(9p) shifts by ≤ 5e-5 at the 34 points, −0.017 at [0,0] where mixing covers. **The stake (P6 failed for a wrong premise):** the 0.19 % radiative share of the bundled loss is the grid maximum at 1e12; at the 34 points it is 5e-8 to 5e-6, yet s = 0 moves u_CRE by −1.87 % and f₃−f₄ by −1.25 % at (9,6), s_max by +21 % and +13 %, τ_slow < 0.01 %. Budget: the net flow into the ground state is 2e-5 of the recombination source at (9,6) (nearly every recombination is re-ionised from an excited state) and the bundled radiative flux is 0.27–0.73 of that net flow, landing 23 % in 1s, 13 % in n = 2; removing the 1s and n = 2 decays alone gives −1.76 of the −1.87 %; a₃, a₄, c₃, c₄ move ≤ 0.2 %, the effect is on u itself.

**Reading.** The chapter's mechanism is refuted and its conclusion survives for a different reason: not ℓ-blindness of the rates but detailed balance among shells that are thermalised (Boltzmann with each other to 6e-5, Saha with the continuum to 1e-4) at every one of the 34 points. Caveats: R3 omits ℓ-summed losses and feeds; 61–63 % of the n = 8 gain in R4 arrives from n = 9 by detailed balance of ℓ-summed rates (statistical by construction), closed from the other side by R3 and b_Saha; nothing resolves ℓ inside n = 11..15; electron impact only.

**Thesis home.** ch6 sec:bundling_check closing paragraph (mechanism replaced, numbers requoted, artifact cited); ch4 sec:ladder summary sentence ("rests on the ℓ-blindness" → detailed balance among Saha–Boltzmann shells, no grid point escapes both criteria). Guard: 0 lost, 36 gained, all from `validation/l_blindness/l_blindness.txt`.

**Still open.** B7, `diagnose_gate_d.py` and `compute_lmix.py:150` as in K13 (author's decisions). The ℓ-blindness item is closed.

### K21. The operator at an arbitrary Tₑ, and the physical ramp L[Tₑ(t)] measured rather than bracketed ✅ Verified (22 Sep 2026)

**Why.** K15 and K17 left "L[Tₑ(t)] at interior temperatures" open because every rate table is tabulated on the 50-point grid and `build_L` takes an index; K17 could only bracket the interior operator from a two-interval span and estimated the one-interval error as a quarter of it. An Explore agent mapped the pipeline: every ingredient is a closed-form function of Tₑ or a Maxwell average of a stored energy-resolved cross section, and every rate module except `compute_K_CCC.py` (which reads, computes and writes at module scope) is import-clean.

**`build_L_at_Te.py`** (library + CLI, `--gate`). Re-implements `compute_K_CCC.maxwell_average` (constants copied, 5000-point linspace, `np.interp`, trapezoid) and the `assemble_K_exc.py` merge rules; imports `compute_K_VS`, `compute_K_TICS`, `ionization_rates`, `recombination_rates`, `compute_lmix` and `assemble_cr_matrix.build_L`/`build_source` read-only, feeding one-Tₑ slabs. Gate: every stored table (`K_exc_full`, `K_deexc_full`, `K_ion_final`, the four α arrays, `K_lmix`, the two CCC tables), `L_grid` and `S_grid` reproduced at all 50 × 8 nodes with difference **0.0**. Hostile audit (math-auditor + verifier roles): builder correct between nodes (nothing interpolated; continuity 1e-9 at nodes; no sparsity or sign switch inside any of the 49 spans; VS clamps never active; tail beyond E_max ≤ 4e-41); the copied-constants argument sound because the average is branch-free in Tₑ.

**`verify_interior_operator.py --write`** → `validation/interior_operator/` (log, 392-span csv, trajectory csv). Gates: G0 the builder's; G1 `verify_ramp_plateau.integrate` reproduces `ramp_plateau.csv` at [15,3] (0.9995/0.9954/0.9555/0.6469 linear, 0.6576 log-linear) to 1e-15; G1b `integrate_path` equals `integrate` to 0.0; G2 builder at the nodes = `L_grid` (0.0), every interior operator a CR generator (column sums 2e-16). *Predicted (quarter rule from K17):* [15,3] 1s-column median error log-linear 0.17 % (accept 0.10–0.26), linear 0.59 % (0.35–0.90); τ_slow +0.075 / −0.71 %; true plateau fraction at De = 1 0.6570 ± 0.0010 inside [c, b]; |Texp − T| < |b − c|; M = 33 vs 17 < 1e-5. *Found:* **P1, P2, P4, P5 reproduced**: 0.177 / 0.626 % (signed −/+), +0.0757 / −0.7649 %, +0.0366 / −0.6833 % in u_CRE; quarter-rule ratios 0.251–0.271 in every compared class at [15,3] and [0,0]; grid-wide 1s-column median 0.116 % (log-linear) and 0.164 % (linear), cold corner 0.360 / 3.610 %. True ramp (Tₑ linear in t) reaches **0.9956 / 0.9567 / 0.6548** at t_ramp/τ_slow = 1e-2 / 1e-1 / 1 against the stamped linear 0.9954 / 0.9555 / 0.6469; with ln Tₑ linear in t, 0.6571. **P3 not reproduced** at De = 1 (0.6548 vs 0.6570 ± 0.001): post hoc, the stamped "log-linear" ramp is, in operator space (to 7e-15), the log-linear interpolant of an exponential ramp, so K17's bracket [0.6469, 0.6576] spans two ramp shapes; each shape's true value lies inside its own interpolant bracket, |T − b|/|b − c| = 0.058 at all three ratios (K17 estimated 0.052). The stamped gap 0.0107 splits into 0.0079 linear-interpolation error, 0.0005 log-linear error and 0.0024 ramp shape. Refuters (T outside [c, b] by > 0.001; one-interval error above half the two-interval) absent. Audit demanded and got four changes: the bracket claim narrowed (184 of 392 spans have 1s-column entries outside it; 0 % above 7.9 eV where both interpolants under-estimate), the "yes to 0.0005" split three ways, "every class" narrowed to the classes compared, the P3 explanation marked post hoc, and a new finding (K22).

**Thesis home.** ch5 sec 5.8.2 (the "not computed" paragraph replaced by the measurement; the K17 two-interval numbers 0.69 / 2.3 / 0.9961 / 0.9620 / 0.6826 / 0.9621 / 0.6834 / 0.9959 / 0.9599 / 0.6671 deliberately removed as superseded, retained here and in `validation/physical_ramp_bound/`); "Under the two operator ramps and the true ramp tested". Guard: 11 lost (listed), 24 gained, all from `validation/interior_operator/` and `validation/maxwell_grid_convergence/`.

**Still open.** The physical Tₑ(t) shape is what no 0-D model supplies. B7, `diagnose_gate_d.py`, `compute_lmix.py:150` (author's).

### K22. The 5000-point Maxwell-average energy grid: its contribution to every thesis quantity, stamped ✅ Verified (22 Sep 2026)

**Why.** The hostile audit of K21 rebuilt the CCC Maxwell average on 25000 points and found τ_slow −0.84 % at the ramp point, larger than the interpolation error K21 measures; chapter 2 sec:maxwellian had tested only the coarse direction (500 points, ~17 % at 1 eV) and called the grid "measured, not defaulted". An audit scratch number cannot enter the thesis.

**`verify_maxwell_grid_convergence.py --write`** → `validation/maxwell_grid_convergence/` (log, 1200-row points csv, transitions csv). Method: the builder's CCC grids rebuilt from the raw CSV at N = 5000 / 25000 / 100000 with the identical rule, in chunks; L and S assembled at all 400 nodes for each N. Gates: G0 the builder's; G1 the N = 5000 rebuild equals `K_CCC_exc_table.npy` and `L_grid` exactly. Mechanism stated first: raw data begin 0 to 0.062 eV above the quantum-number threshold, `np.interp` returns 0 below them, the trapezoid resolves that step to one spacing (0.19 eV at N = 5000). *Predicted:* [15,3] τ_slow −0.5 to −1.2 %, u_CRE −0.5 to −1.1 %, 1s-column median 1.0–2.5 %; 250–500 transitions > 2 % and 15–60 > 10 % at 2 eV, all > 10 % ones below 1e-3 of the strongest rate from their initial state; effect grows toward 1 eV; 25000 → 100000 step below a quarter of 5000 → 25000. *Found:* P1, P3, P4 reproduced (−0.875 / −0.794 / 1.611 %; [0,0] −2.95 % > [15,3] > [49,7] −0.088 %; further step ratio 0.10); **P2 not reproduced** on its last clause: 383 > 2 %, 34 > 10 %, but the > 10 % ones reach 1.3e-3 of the strongest rate (3.4e-2 at 1 eV). N = 100000 against 5000: [15,3] τ_slow −0.96 %, u_CRE −0.88 %, 1s column median 1.94 %; benchmark −0.56 / −0.51 / 1.18 %; cold corner −3.34 / −3.23 / 4.25 %; over Tₑ ≥ 2 eV the worst shifts are τ_slow 1.53 %, u_CRE 1.32 %, f₃−f₄ 0.81 %, ε_plateau 0.97 %. *Refuter (a thesis quantity moving > 2 % at Tₑ ≥ 2 eV): did not appear.* Nothing repaired: `N_GRID` stays 5000 and every stored table is unchanged.

**Thesis home.** ch2 sec:maxwellian (grid "measured in the coarse direction", fine-direction numbers added with the mechanism); ch5 sec 5.8.2 caveat requoted from this run. Reported to the author: `compute_K_CCC.py`'s "validated: <2% error" comment does not hold for the weak transitions (34 of 1320 move > 10 % at 2 eV).

### K23. Round 6 (introduction, framing, novelty, conclusions) review adjudicated; chapters 1 and 7 requoted from stamped artifacts ✅ Verified (22 Sep 2026)

**Why.** An external Round 6 review of ch1 §§1.1–1.9 and ch7 §§7.1–7.5 (verdict FAIL; 35 claims after splitting) was adjudicated with the review-audit skill by three read-only subagents (chapter 1; ch7 §§7.1–7.3 with the sign columns of `reservoir_gain.csv` and `operator_slope_decomposition.csv`; ch7 §§7.4–7.5 against K13/K14/K18): 33 correct in some degree (8 already applied before this round, 3 understated, 8 minor, 7 partial), 1 wrong (the ADAS reservoir check it called missing was stamped in K13), 1 not a physics claim, 0 fabricated. The reviewer read a 213-page build two rounds old. Full table: `outputs/round6_adjudication_2026-09-22.md`.

**Understated by the reviewer.** (5) Chapter 1 asserted the ELM as a step; K15's exposure census has τ_slow below 100 µs at 237 of 448 defended pairs and the 45 census members all above 195 µs, so over much of the range the reservoir is faster than the ELM rise; ch1 now idealises the step and defers the ordering to ch5. (7) Fujimoto's Appendix 4B treats a time-varying n(1) and defers its dynamics to Appendix 5B, so "the frozen reservoir is how the standard textbook treatment is posed" contradicted the cited source; requoted. (12) ch7 §7.2 said "their product is positive for heating and negative for cooling" of S̄ and G, both negative on all 2288 rows; S̄G > 0 always and S̄GΔln Tₑ carries the sign; requoted with the orientation of eq. Sbar_def named.

**Applied (science-editor role, ch1 11 edits, ch7 21 edits, appendix C 1, guard clean, zero dashes, every label resolves).** ch1: chord average of n₃/n₄ stated as the uniform-volume (0-D) case; opening reads Tₑ at supplied density; "neither carried as an unknown nor supplied from outside"; ELM step idealised (ch1:687, 877), "rises in a few hundred microseconds"; Greenland heading a criterion not a verdict; §1.9 "temperature inversion at known density"; light "localises nothing along its chord"; probe claim "not supported by the design analysis"; the slow clock's barrier named as 10.2 and 13.6 eV, both several times k_BTₑ. ch7: Fujimoto gap restated; negative novelty claim search-scoped; "part in 10⁵ at the benchmark and 4.4e-5 everywhere analysed"; eigenvalue argument conditioned on S ≠ 0 with the two excluded points (sec:attacks); "fails by an amount that decays only on τ_slow"; S̄ orientation and the S̄GΔln Tₑ sign sentence; forward-criterion sentence; G as a secant in the procedure; "moves the line ratio by 3.5 % over the heating pairs", amplification 8.3 (K8); |G| 2.640 to 7.822 "over the heating pairs"; single exponential "very nearly", 2.2 %/3.8 %; census 166 + 44 (all heating, `inversion_error.txt`) + 238; joint-step qualifier "one-index density step, a factor 2.68"; "densities of this model's defended range"; closing paragraph: known density, 0-D closed reservoir, S̄ step-dependent by 22 to 30 % at the benchmark (`reservoir_gain.csv` k = 1, 2, 4); §7.4 structural results named (two-channel split, logistic form, tanh ceiling) with the sampled S̄, G and ridge conditional; Tₑ ≥ 2 eV as the working domain the two contours support (1.41–1.50 eV, 1.01–2.36 eV, ch5 §5.10.3); molecular paragraph "a sensitivity scenario and not a bound"; "A density grid as fine as the temperature grid"; H₂ bound survives at fixed v with a v-dependent Δ (checked numerically); the fixed-density inference requoted beside the self-consistent 67 and 7 (K18); one sentence citing the ADAS reservoir check (1.18, 5.8 %, sec:gate_d). Appendix C: G, S̄ and ε_plateau not scanned directly, their ingredients converged (Δln u ≤ 2.3e-5 between n_max 14 and 15).

**Not acted on.** Re-weighting §7.3 toward error separation, the crest mechanism and Σ = P + SG (line count: bound 74, reservoir-vs-QSS 58, factorisation 7, crest 0, P + SG 0): a restructuring under the page budget, the author's call; the closing paragraph was reworded instead. The section grades.

**Thesis home.** ch1 §§1.1, 1.3, 1.4 opening, 1.5, 1.6, 1.7, 1.8, 1.9; ch7 §§7.1–7.5; appendix C state-space convergence.

### K24. The Saha-balance ladder check stamped: 0.999997 for all 43 states at all 50 temperatures, spread 1.4e-15, the residual the rate module's rounded constants ✅ Verified (22 Sep 2026)

**Why.** Chapter 4's "one energy ladder" sentence (sec:one_ladder, Table 4.4 row "One energy ladder on both sides", graded severe) quoted a Saha ratio of 0.999997 "to the same eight digits" from a working note with no producing script; the master-register audit (M19) flagged it as the last eight-digit claim without provenance.

**`verify_saha_balance.py --write`** → `validation/saha_balance/` (2150-row csv, log). Method: ratio = α_3BR / (K_ion × (g/2) Λ³ exp(I/kT)) with CODATA 2018 constants (h, mₑ, eV) against the module's rounded ones; I = IH/n² with IH the 1s entry of `state_index.csv` (which stores I_eV rounded to 8 decimals; that rounding alone would contribute 5e-9 through exp(I/kT) and is reported, not absorbed). Gates: G1 the module's own factor reproduces α_3BR/K_ion to 7e-16; G2 the CCC thresholds equal I_eV[n_i] − I_eV[n_f] to 5e-9 eV on all 1320 rows (one ladder). *Predicted:* 0.999997 ± 2e-6 everywhere; spread < 1e-8; residual = (h_mod/h)³(mₑ/mₑ,mod)^1.5(eV/eV_mod)^1.5 to 1e-9. *Found:* 0.999997390 at every entry, spread 1.44e-15, residual/explanation − 1 = 0.0. P1–P3 reproduced; refuter (an entry off the median by > 1e-6) absent. Reading: the closeness to unity is detailed balance by construction; what the sentence rests on is the identity of the residual across the full binding-energy ladder, and that identity is now measured to 1e-15. The residual is immaterial and reported, not repaired.

**Thesis home.** ch4 sec:one_ladder sentence requoted with the artifact; Table 4.4 row provenance "notes" → "stamped" (one severe row still rests on notes).

### K25. Master correction register (Rounds 0–10) adjudicated; the six residual synchronisation sentences, three captions and PDF metadata applied; prose cleanup of draft history, internal file names, reader address and detective headings across all chapters ✅ Verified (22 Sep 2026)

**Adjudication** (`outputs/master_register_adjudication_2026-09-22.md`): 36 MUST items, 27 already applied in K11–K23, 6 with residual sentences applied today (M10 ch7 opening; M11 abstract and ch5 summary census 166 + 44 + 238; M13 ch5 summary ceiling versus dataset cap; M14 two captions; M16 a dangling word; M23 the Fig 5.8 caption; M36 ch5 conditional sentence, "worst heating pair", ch6 "108 pairs", ch7 §7.4 Fujimoto factor 8.3), M22 superseded by K21, M19 stamped (K24), M28 and M31 the author's, M7 and M18 not defects. SHOULD: S15 applied (`\hypersetup`), S1/S4–S6/S8–S10/S12/S13/S16 already done, S7 not a defect, S2/S3/S11/S14 the author's. PAPER-LATER: P1 (ADAS reservoir, K13), P2 (K14, K23) and P3 (K21) are done; the register was written from a build two rounds old. Caption edits made in both the `.tex` and the generator templates (`make_story_figures.py`, `make_ch5_figures.py`), as K17 requires.

**Writing practice.** The author asked what thesis-writing guidance says about the phrasing the register's cleanup list targets. Sources read: Lund Mathematical Physics "How to write a thesis" (dry, correct, objective; do not list trials chronologically); Coventry CAW "Style and Register" (no direct address to the reader); Thomson, "Don't give your thesis examiner a bad first impression" (generic headings, cluttered abstract); the examiner study "Advice for writing a thesis (based on what examiners do)" (coherent whole, headings that identify what follows); the IIT Kanpur thesis guide (format only, defers style to standard manuals). Inventory before (live lines): "earlier draft" 52, "earlier version" 7, `findings_10` 23, `ADDENDUM` 12, `thesis_ready` 3, `pivot_decision` 1, "the reader" 9, "It is worth" 16, "obvious" 15, "honest" 11, "attack" 12, "A chemical engineer" 6, "suspect" 5, and 14 detective or "The question"/"The picture" headings. **Cleanup** by science-editor subagents per chapter (ch1–ch7, appendix C) under a hard rule: every number, caveat, refuter and provenance pointer survives (per-file numeric-token guards; the only lost tokens are digits inside deleted internal file names and superseded working-note values whose stamped replacement stays in the same passage); draft history recast impersonally, instructive wrong values kept as methodological remarks; working-note citations become "(project working note, no producing script; Table 4.4)" so the provenance disclosure stays; reader address and rhetorical words replaced; 24 headings renamed descriptively (ch1 §1.8 "The research question and the hypothesis" and nine run-in labels; ch4 "Two tests of the reduction, and their expected outcomes"; ch6 twenty subsection and section titles, e.g. "The open ion boundary: a closed-parcel test"; ch7 "The excited-state closure holds; the reservoir closure does not" and "A forward criterion for the equilibrium lookup"). Labels unchanged (`sec:attacks` etc. remain as labels only). After: every counted term at 0 in live prose except the label text.

**Not done, the author's.** M28 (one writer per timescale array), M31 (name the independent party: if AI-assisted, say "an independent re-implementation with an AI assistant"), M32 as a from-scratch rewrite, S2 (`validation/MANIFEST.csv`), S3 (clean regeneration), S14 (schematic), the page target.

### K26. Round 7 (adversarial thesis-wide kill test) adjudicated; Park 1972 read and conceded, the chord-integration scope corrected, and a step-idealisation overstatement found by auditing a clearance ✅ Verified (22 Sep 2026)

**Why.** An external adversarial review returned FAIL on six chapters with five MAJOR items. Adjudicated with the review-audit skill by four read-only subagents against the current text and the stamped artifacts, with the reviewer's favourable claims audited at the same standard. Full table: `outputs/round7_adjudication_2026-09-22.md`. Outcome: 1 correct and understated (Park), 1 correct (chapter 3 chord cancellation), 2 partially correct (Fujimoto wording, provenance), 1 wrong as stated (abstract conditionality, already applied in K18 and K23), 1 already applied (console criterion). No fabrications.

**Park 1972, settled from the primary source.** The author downloaded the paper mid-session (`refs/0022-4073_2872_2990050-7.pdf`, JQSRT 12, 323 to 370, received 26 May 1971, Ames Research Center); Crossref confirms the record. It was absent from `references.bib`. The omission is larger than the reviewer knew: Park assumes quasi-steady state within the excited levels (his assumption d, with the timescale-separation justification), writes the same two-coefficient decomposition ρ(p) = r₀(p) + r₁(p)ρ(1), and carries the normalised ground-state population ρ(1), which is this thesis's b₁, as the third argument. The boundary is sharp and comes from Park's own words: he computes only in the two limits he calls *terminal* nonequilibrium, decaying (r₀ dominates) and rising (r₁ρ(1) dominates), named terminal "because it represents the condition in which any further departure from equilibrium does not affect the emission ratio of hydrogen lines". That is precisely the vanishing of the sensitivity of ch3 sec:ground_fed_fraction, which the thesis independently proves "vanishes in two limits, and only there". Checked on the model's own CRE populations: over the 400 grid points f₃ runs 0.055 to 0.943, f₄ 0.009 to 0.796, the difference never below 0.046 (already stamped at ch4:1002, K13) and above 0.1 at 343 points; benchmark f₃ = 0.260, f₄ = 0.048 reproduce the printed values. **Zero of 400 points lie in Park's regime.** Applied: bib entry `Park1972`; a ch1 sec:prior_work paragraph conceding the idea, the QSS assumption, the decomposition and the parameter; a ch7 sec:novelty paragraph drawing the boundary.

**Chord integration (claim 1).** The defect was real and this session's Round 6 pass had fixed only the ch1 instance. ch3:942 said the line-integrated brightness turns into a ratio "in which both cancel" and then defined R = n₃/n₄; ch3:1207 said the n_ion prefactor cancels "as does the emitting path length". The chord ratio is the n₄-weighted average of the local ratio and equals it only where that ratio is constant over the emitting region; calibration and the common scale do cancel, the profile does not. Both requoted. Chapter 6 had **no** line-of-sight limitation (zero hits for chord, sightline, brightness, spectrometer); a new subsection `sec:los_limitation` was added at the end of sec:transport, stating that the weighting is by Hβ emissivity rather than path length and that converting the point result into a chord result needs an emissivity profile, which is a transport result. Severity moderate, not MAJOR: no reported number is a chord quantity.

**Found by auditing a clearance the reviewer granted.** The reviewer cleared the ramp objection. Chapter 6 said the step idealisation is "safe by one to two orders of magnitude where a plateau is reported, not by three", while its own stamped range over the 45 breakdown pairs is D_e = 1.95 to 288, median 11.2. Evaluating the chapter's own lag law D_e(1 − e^(−1/D_e)) gives 0.78, 0.96 and 0.998: at the least favourable pair the ramp reaches 78 % of the step plateau, a 22 % suppression comparable to the errors reported. Requoted with the three figures and the arithmetic disclosed as arithmetic on stamped ratios, not a separate run.

**Fujimoto (claim 2).** Numbers exact (8.29 and 4.39 low; cap +28.6 %, sensitivity +29.5 % median at the 1e12 scenario), but one of two scenarios: at the highest density the deficit is nearly common and the cap moves −3.8 %. The status is already open at length in ch4 and ch6, so the claim is not new. What was genuinely too strong is one sentence conflating benchmarked **input** rates with a benchmarked **derived** coefficient; ch4 now reads "The input rates are traceable and benchmarked where the observable lives; one derived coefficient there is not", with the open r₁ deficit and its consequence for a₃, a₄ named.

**Provenance (claim 5): partially correct, consequence refuted.** Both writers confirmed (`qss_analysis.py:411-413`, `validate_gates.py:462-464`, same three paths, no producer field), but the arrays feed no thesis number: all six readers write figures and files absent from `thesis_tex/`. The abstract's 9982 comes from `verify_timescales.py` via `validation/spectrum_testpoint.csv` and `validation/timescale_verification.csv` and recomputes bitwise from the canonical L_grid. A latent hazard, correctly disclosed. Repair remains the author's.

**What the review missed.** The abstract is about **734 words against the IIT Kanpur M.Tech limit of 300** ("not exceeding 300 words, about one page"). A formal submission requirement, missed by a factor of two and a half, raised by no review in this series. Nothing was added to the abstract this round for that reason; the cut is the author's decision.

**Guard and build.** 12 lost tokens (the superseded ch5 and ch6 values of earlier rounds, replacements in place), 137 gained, all traced; one apparent gain, "972" in ch1, is a guard artifact from the citation key `Park1972` and not a thesis number. 210 pages, 0 LaTeX errors, 0 BibTeX warnings, 0 undefined references, 0 em dashes outside comments.

### K27. Round P0 (provenance and regeneration) adjudicated; 45 printed numbers re-derived from stamped artifacts, Table 4.4's ledger made self-checking, and 24 unstamped artifacts found ✅ Verified (22 Sep 2026)

**Why.** An external provenance review returned FAIL with two MAJOR defects, auditing the PDF only (it says so) without repository access. Adjudicated with the review-audit skill by three read-only subagents tracing each claim from the thesis sentence back to its producing artifact, and auditing the review's own PASS verdicts at the same standard. Full table: `outputs/roundP0_adjudication_2026-09-22.md`. Outcome: 4 correct, 2 correct and understated, 1 partially correct with its severity refuted, 1 set of unearned clearances. No fabrications.

**P0.1 contamination refuted on all six named results.** The dual-writer defect is real (`qss_analysis.py:411-413` and `validate_gates.py:462-464` write the same three paths) but reaches nothing printed. M = 9982 is τ_slow/τ_relax computed from L_grid by `verify_timescales.py` into `spectrum_testpoint.csv`; the factor 19.3 comes from `m_rank_test` via `divertor_map.csv`; window membership is recomputed from `eigvals(L)` inside the producing script; `verify_transport_selection.py` takes τ_QSS from `divertor_map.csv`; the thesis has no M-map figure. Both writers sort the eigenvalues of the same canonical L and keep the negatives, neither applies the historical `< -1.0` filter (CLAUDE.md's known-issues row for `qss_analysis.py` is stale on this, confirmed a second time), and the arrays on disk agree with the independent artifact to 2e-16. ch4's disclosure now carries both bounding facts.

**`verify_headline_provenance.py --write`** → `validation/headline_provenance/` (45-row csv). Re-derives every printed number that carried a working-note citation: the two QSS errors and their CRE pairs, tab:gain's nine gains, tab:closure's twelve entries, tab:position_effect's twenty. **All 45 reproduce to the precision printed.** No new physics: each value is a reduction of an artifact produced by another script, named per row. Two findings came out of building it. (i) The QSS errors were never unstamped: they are `max_track_w30_shell` at [23,5] and [0,4] in `trajectory_census.csv`, 8.65971e-6 and 6.72618e-9, and looked unstamped only because the artifact calls the quantity `max_track`. (ii) tab:position_effect's `ln(u_CRE/u_peak)` takes its numerator at Tₑ index 23 and its denominator at index 24, a single logarithm of a ratio at two temperatures, while the caption said index 24; evaluating it consistently at either index shifts every entry by about 0.25 and moves neither the sign change (between j = 2 and j = 3) nor the maximum of |S̄| at j = 3. The caption now states the convention and the size of the alternative.

**`verify_ladder_counts.py`.** Table 4.4's caption states six counts over its own rows. The script parses the rows and checks all six, exiting non-zero on drift. It reproduced the five correct ones and caught the sixth: the caption said four severe checks rest on notes. At HEAD exactly one did, so the sentence was already wrong by three; K24's Saha stamp made it zero. Corrected, and the caption now names the guard.

**Found by auditing the review's PASS verdicts: 24 of 97 validation CSV files carry no header at all**, no producer, no interpreter, no input hash. Among them the artifacts behind the reservoir gain, transport selection, joint-step map, emissivity generalisation, fault injection, crest subgrid, operator conditioning and Lyman trapping census, several of which underwrite abstract-level numbers the review marked PASS. The producing scripts print the hashes to stdout; they were never written into the files. Broader than the dual-writer defect and invisible from the PDF. Reported, not repaired: writing the headers means re-running the producers.

**Applied.** Five re-citations (tab:gain, tab:closure, tab:position_effect, the ch5 QSS sentence, the abstract), which also removed the last internal file name from live prose. Guard: 13 lost (12 superseded values of earlier rounds plus "09" from the removed `findings_09` citation, a guard artifact of the underscore-digit filename), 143 gained.

**Still open, the author's.** Headers for the 24 artifacts; one writer for the three timescale arrays; the eight working-note numbers needing a run, of which the load-bearing are |λ₀| = 2.286 K_ion nₑ, the eigenvalue conditioning and single-precision test, and the ℓ-weighting invariance of Δ. Two third-digit disagreements found in the inventory and not yet resolved: the caps behind the 51 % and 97 % utilisation figures (printed 0.4503 and 0.4394 against stamped 0.451357 and 0.441707) and the crest vertex (printed 3.66e13 against `crest_subgrid`'s 3.7047e13).

### K28. Round P0 re-check adjudicated; every remaining working-note number computed from the canonical operator and re-cited ✅ Verified (22 Sep 2026)

**Why.** A follow-up provenance review confirmed the five earlier P0 repairs and downgraded the dual-writer defect to MODERATE, but found that active numerical claims still carried "project working note, no producing script" against the Declaration's standard. The author's instruction: every number from a run and a citation. Full adjudication: `outputs/roundP0b_adjudication_2026-09-22.md`. Verdicts: 5 correct, 1 correct and understated (it treated the repair as costly; it was not), 1 partially correct (the Declaration's opening clause was again elided).

**`verify_residual_claims.py --write`** → `validation/residual_claims/`. Computes fifteen groups of claims from `L_grid`/`S_grid` or from stamped artifacts, gated on reproducing CLAUDE.md's benchmark timescales. **All 43 compared values reproduce at the precision printed**, including several that had to be reconstructed from an unrecorded definition:
- the three slowest eigenvalues at [1,4] and the 3.7e7 gap (−4.287, −1.603e8, −3.443e8);
- the bound population 8.925e-4 per n_ion and the 0.09 % cost of freezing n_ion (8.924573e-4, 0.0892 %);
- **all five ℓ-weighting values of Δ to five decimals** (1.94561, 1.94474, 1.94447, 1.94471, 1.94574) from a reconstruction of which five weightings were meant, spread 0.0652 % against 0.07 %;
- **the four eigenvalue condition numbers exactly** (1.707, 1.981, 3.255, 2.589) under 1/|yᴴx| with unit-norm vectors, plus the float32 displacement 1.356e-5 against 1.3e-5, a definition the thesis had not recorded;
- the cap utilisations 50.8 % and 97.0 %, with the cap at the post-step index;
- the ℓ-mixing bounds (0.151 % and 0.139 % at factors 3 and 7, inside the stated 0.85 % and 1 %);
- the per-row argmax and 3 of 49; the (3,5) peak at 7.2e12 and factor 2.683; f₃ = 0.0713, f₄ = 0.0109 at [23,7]; the cap span 0.394 to 0.450 over the table's sampled columns; the 54 no-window heating pairs, all at j ≥ 4;
- **the ion-closure census exactly**: 202 → 200 above the 10 % criterion, worst case 38.68 % → 38.56 %, and 45 of 448 unchanged above 2 eV.

**One real error found, not merely a provenance gap.** The sub-grid crest sentence printed 3.66e13 at 1 eV and 1.51e13 at 9.5 eV where `crest_subgrid` gives 3.7047e13 and 1.5233e13. Worse, the quoted factor 2.56 is right but is max over min across all temperatures, not the ratio of those endpoints (2.42), and **the drift is not monotone**: the vertex falls to 1.4472e13 at 5.96 eV and rises again, one sign change. The sentence claimed it "drifts downward with temperature". Requoted from the artifact with the reversal stated; the paragraph's conclusion (the drift is under one grid interval and unresolved) survives.

**Two definitions not recovered, reported rather than forced.** The matched heat/cool ratio 1.036 (this reconstruction gives 1.111 dividing each error by its own |Δln Te|) and the linearisation median 1.031 (1.018 over the 392 heating steps, while the range, the 219 count and the benchmark 0.9459 all reproduce). Both sentences now say so in the text rather than citing a note. Adjusting either construction until it matched would have been making a check pass.

**Self-correction to K27.** K27 listed the caps behind the 51 and 97 per cent figures as a third-digit disagreement. That was wrong: 0.4503 and 0.4394 reproduce exactly at the post-step index (0.450299, 0.439352), which is the index `tab:position_effect` uses. A subagent had compared at the pre-step index and it was not checked before reporting.

**Final state of the phrase.** From 23 live occurrences to 7: five now read "a historical project value with no producing script, not used as evidence here and superseded by <the stamped run>"; one is the Table 4.4 caption defining the term; one (ch5:931) is a structural claim about the logistic width whose only numerals are the subscripts of a₃, a₄, c₃, c₄. No number in the thesis now rests on an unscripted note. Guard: 15 lost (13 prior plus the two wrong crest values), 157 gained. 211 pages, 0 errors, 0 undefined references. All three provenance guards pass.

### K29. The abstract cut to the institute limit ✅ Applied (22 Sep 2026)

**Why.** The IIT Kanpur thesis guide limits an M.Tech abstract to 300 words, about one page. The abstract stood at roughly 800 by a LaTeX-aware count (801 counting `\SI{}{}` as value plus unit), and no review in the series had raised it.

**What was kept.** Every scope statement the review rounds installed: the inversion is for temperature **at known density**; the bolded disclaimer that this is the cost of the closure where it is adopted and **not a demonstration that divertor spectroscopy fails**; the reversal (the excited-manifold QSS closure holds to 8.66e-6, the ground state is what lags); the exact factorisation and the universal ceiling |f₃−f₄| < 1 attributed to the structure of the linear system rather than to any rate coefficient; the 45 of 448 census and worst case 17.5 %; the full inversion split 166 + 44 + 238 with the 48 % median; the two measured boundaries below 2 eV; and the closed-parcel conditionality with "the structural results survive that; the magnitudes are conditional on it".

**What was dropped to the chapters.** The 506 µs census (24, 15.3 %), the exposure-integrated census (43), the G drift (6.74 %) against the ε factor (8.44), the joint-step stress test (22.1 %), the detached subset (108 pairs, 7.2 %), the benchmark M (9982) and the factor 19.3, and the sharper dataset-specific cap tanh(|Δ|/4). **The number guard confirms none of these was lost from the thesis**: LOST stayed at 15 and GAINED at 157, unchanged by the cut, so every dropped figure still appears in a chapter and no new number was introduced.

**One regression caught and fixed.** The first cut reduced the timescale-separation sentence to "predicts none of it", dropping the "licenses the reduction" clause that K23 had installed for M18 (M supports the fast-state reduction but does not rank the reservoir error). Restored.

**Result.** 270 words as rendered, on one page, 210 pages total (one page shorter), 0 LaTeX errors, 0 undefined references, 0 em dashes. All three provenance guards still pass.
