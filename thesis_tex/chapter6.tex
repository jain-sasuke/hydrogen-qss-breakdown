...
the plasma is most neutral where it is coldest. At the worst point of the error
map --- $\Te = \SI{1.0}{\electronvolt}$,
$\nel = 5.18\times10^{13}\,\si{\per\cubic\centi\metre}$, which is the maximum of
$\epsplat$ over all $680$ window-resolvable points of the map, cold edge
included --- the model's own CR solution gives $\ngs/\nion = 28.3$, so
...
Chapter~\ref{ch:results} --- $45$ of $448$ points above \SI{2}{\electronvolt},
...
over $108$ points is \SI{7.2}{\percent} --- is now known to be \emph{invariant}
under the one physical process most likely to have invalidated it. That is a
...
\begin{table}[tbp]
  ...
  point of the optically thin error map --- $\Te = \SI{1.0}{\electronvolt}$,
  $\nel = 5.18\times10^{13}\,\si{\per\cubic\centi\metre}$, heating step; the
  maximum of $\epsplat$ over all $680$ window-resolvable grid points --- traced
  as a function of the assumed slab thickness $D$. $\epsplat$ is the plateau
...
ionisation; the ground-state population falls --- at
...
$D = 1$ and \SI{20}{\centi\metre} --- and the slow clock $\tauqss$ shortens by a
factor $8$ at the worst point. A reservoir that turns over faster is a less
...
a worst case of $0.2550$ to $173/680$ and $0.2653$. Continuum lowering, and
opacity in the Balmer lines themselves --- which would act on the observed
ratio directly rather than through the level populations --- are untouched here
and remain open.
...
surface when ions strike it and recombine there --- this is \emph{recycling} ---
...
The size of what is being left out is documented experimentally. In
spectroscopic inversions of divertor Balmer emission at the onset of
\emph{detachment} --- the operating regime in which the plasma cools enough that
the ion flux to the target collapses, and the regime this diagnostic is chiefly
used to identify --- molecularly-induced emission is found to account for up to
...
not turn on either subtlety --- the ridge is below both --- but the factor
...
passes. It is --- for one of the two coefficients. This section reports the
other, which fails, and does not repair it.
...
The recombination-fed coefficients agree: $r_0$ matches to
\SIrange{0.1}{1}{\percent} for $n \ge 4$. The ground-fed coefficients at low
principal quantum number do not. Against Fujimoto's Table 4.1(b), at
$\nel = 10^{12}\,\si{\per\cubic\centi\metre}$, this model's $r_1$ sits a factor
$8.3$ low at $p=3$ and a factor $4.4$ low at $p=4$.

Those are exactly the two shells whose difference is the mechanism of
Chapter~\ref{ch:results}, and
$\nel = 10^{12}\,\si{\per\cubic\centi\metre}$ is one grid interval from the
ridge. This is not a discrepancy in a corner of the model that nothing depends
on.

The obvious suspect has been eliminated. If the excitation rates feeding the
ground-fed channel were wrong, the deficit would follow; but those rates were
benchmarked independently against R-matrix-with-pseudostates (RMPS) calculations
and the dominant $\Delta\ell = 1$ channels agree at a median of
$-\SI{5.0}{\percent}$ over $120$ comparisons. A factor of $8$ is not hiding
there. No alternative suspect has been identified.

\subsection{What it costs, and what it does not}

The consequence is bounded in one direction only, and the bound comes from
Section~\ref{sec:ridge_conditional}: since the ridge location responds roughly
decade-for-decade to the strength of the ground-fed supply, a factor-$8$ error
in $r_1(3)$ is not obviously harmless to where the ridge is. The ridge
\emph{mechanism} --- that the two shells switch supply channels at different
reservoir strengths, and that the switching point moves with the shell pair as
Griem's criterion predicts --- depends on the \emph{ratio} of the two shells'
responses and on how it scales, not on the absolute normalisation, and survives
either way.

This is reported open because it is open.
\todo{Report $\partial(\text{ridge location})/\partial(r_1\ \text{scaling})$
explicitly. If a factor-3 change in $r_1$ moves the ridge by more than one grid
interval, the ridge \emph{location} must be withdrawn and only the mechanism
retained. Unresolved as of this draft.}
