...
\SI{+0.005}{\percent}, so the sign does not depend on that choice. An earlier
draft quoted \SI{+0.9}{\percent}, \SI{-1.4}{\percent} and \SI{-0.55}{\percent}
with a common ratio of about $0.75$ from a working note
(\texttt{findings\_10} ADDENDUM~D.8) that no script produced; the stamped run
reverses both signs, and the sentence built on them. Truncation makes
...
gives \SI{17.9}{\percent}, \SI{25.9}{\percent} and \SI{13.3}{\percent} (an
earlier note recorded \SIrange{22}{37}{\percent}). The jump is an artifact of
