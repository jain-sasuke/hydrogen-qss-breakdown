...
worst point; the measured factor there is $15$. Both numbers belong to
$[0,4]$, at \SI{1}{\electronvolt}, so the two orders of magnitude between them
are a cold-corner margin. Inside the defended range the margin is smaller and
the conclusion is the same: at the ridge the factor needed is $20$, since
$\tauslow$ there is \SI{2.027}{\milli\second} against a
\SI{100}{\micro\second} event, and the measured closure factor is $1.01$; at
the benchmark $\tauslow$ moves by one part in $10^{3}$. The attack was survived everywhere it was pressed, and this
belongs in the defence of the result rather than in a list of caveats.
...
relevant ratio is $\tauslow/\taudrive$. At the worst point of the grid that
...
\SI{72}{\micro\second} across \SI{10}{\centi\metre}, against a local ionisation
balance the model computes for itself. At the worst point the two are
\SI{72}{\micro\second} against \SI{233}{\milli\second}, a factor
$3.2\times10^{3}$, but that point lies below the scope boundary. Inside the
defended range the separation is smaller and does not always keep its sign:
over the $45$ defended breakdown pairs $\tauesc/\tauslow$ has median $0.0079$
and reaches $0.159$ at its largest (Table~\ref{tab:transport_selection}), one
to three orders rather than four, and at the benchmark it is $3.6$, so there
transport is the \emph{slower} of the two. What the model contains is the
ionisation clock alone, whichever way the comparison runs.
...
in $325$; above \SI{2}{\electronvolt} the count is $45$ before and after. At
the cold corner a factor of $2300$ would have been required to refute the
result and the measured factor is $15$; at the ridge, inside the defended
range, the factor required is $20$ and the measured one is $1.01$.
