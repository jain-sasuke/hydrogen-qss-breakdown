...
\paragraph{The point the table belongs to, recovered.} An earlier draft carried
Table~\ref{tab:faultinject} without recording the grid point, and three of the
four $\max|\Delta\Lmat|$ values scale with $\nel$, so the table could not be
reproduced. Sweeping all $400$ grid points and matching on those three values
...
session note, which is not a check; the stamped script reproduces it to
...
and $\ffrac{3}-\ffrac{4}$ jumped by \SIrange{22}{37}{\percent}: an artifact of
...
what is lost is a way out and not, as an earlier draft said, something ``to
cascade out to''). The loss removed is not small. Continuing the $12\to13$, $13\to14$
...
invoking the atomic data (it does not sit inside the $6.3$ to $10.7$ interval,
as \texttt{findings\_10} ADDENDUM~D.8 wrongly states; the defensible claim is
agreement in magnitude and trend). No result in this thesis rests on the
...
and treats the Lyman series, where transparency fails. The optical depth of
$H_\alpha$ across a \SI{10}{\centi\metre} slab is
...
exceeding the duration of the disturbance, the obvious attack is that $\tauslow$
is an artifact of the open boundary.
...
plasma at all at the top read as \si{\per\cubic\centi\metre}. An earlier draft
proposed a one-decade offset in the density rows; that explanation is withdrawn.
\end{enumerate}
...
magnitude, that overstates the table by up to a factor $6.65$. An earlier draft
quoted that read, $323$ of $400$, a range of $0.130$ to $0.931$ and a median of
$0.696$, with a deficit deepening toward low temperature; read logarithmically
...
Earlier drafts read the failure as costing the thesis its only comparison of
absolute effective coefficients against an external database; that comparison
exists, hidden by the definition. The failure does not touch the structural results
...
$2p$--$2s$ value is $6.95$ (Section~\ref{sec:atoms_corrections}). An earlier
draft quoted $6.65$, $5.73$, $4.29$ and $2.92$ from a derivation note.
...
An earlier claim that the frozen Debye density varies the cutoff factor by about
\SI{30}{\percent} across the grid was wrong, the module's own functions giving
...
the one that fails. The error was found by a hostile read of the thesis text
...
Section~\ref{sec:bundling_check} tests that assumption: it is licensed over most
of the grid, but not at the two highest densities below \SI{3}{\electronvolt}
once the cutoff is evaluated locally, where it rests on the $\ell$-blindness of
