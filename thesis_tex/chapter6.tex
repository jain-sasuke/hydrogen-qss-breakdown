...
Chapter~\ref{ch:results} established two things: a map of how wrong a
...
three separate ways. At this point in the argument a result stands.

This chapter's job is to bound that result without withdrawing it. Every number
...
result. Two objections were expected to be decisive and were not; they are given
...
\subsection{The optically thin assumption and how it reaches the slow clock}
...
The failure of this assumption has a direct analogue in reaction engineering.
...
The counts $105$ and $26$ can also be obtained, and are a convention error
rather than a recomputation: they are reproduced only by combining the
full-path depth $\kappa_0 D$ with a Doppler width $\sqrt{kT_n/m}$, short of the
correct $\sqrt{2kT_n/m}$ by $\sqrt{2}$. With the correct width the census is $85$ and
...
Nothing in the defended range depends on the choice of width convention: with
the absorbing atoms at $\Te$, every crossing lies below \SI{2}{\electronvolt}
except two, both at $D = \SI{20}{\centi\metre}$; colder atoms move the
crossings upward, and Section~\ref{sec:scope} gives by how much.

\subsection{Self-consistent Lyman escape factors applied to the matrix}
...
simpler route. The simpler guess is that trapping desensitises the diagnostic,
...
now been shown not to move the defended census within the tested escape-factor
model ($D = 1$ to \SI{20}{\centi\metre}, $\Tn = \Te$). The agreement with
Griem's boundary criterion is therefore not an artifact of neglected opacity; it was established where
...
a worst case of $0.2550$ to $173/680$ and $0.2653$; fixing it at
\SI{0.5}{\electronvolt}, colder atoms returning from a surface, changes them to
$166/680$ and $0.2443$; above \SI{2}{\electronvolt} the count is $45$ of $448$
under all three, with the worst case moving from $0.1746$ to $0.1744$
(\texttt{verify\_lyman\_trapping.py}, \texttt{validation/lyman\_trapping\_tat3/}
and \texttt{\ldots\_tat05/}). Continuum lowering is
...
\subsection{An objection the model cannot answer from inside}
...
\subsection{Batch-reactor parcel against a recycling divertor}
...
set by local ionisation balance alone. It is set by recycling and transport
together with the local atomic and molecular reactions.}
\end{quote}
...
Neutral transport takes the reservoir's history out of the local balance. Any
process that replaces the ground-state population on a timescale shorter than
$\tauslow$ makes $u(t)$ something this model does not compute, and whether the
error then shrinks or grows depends on what the influx supplies
(Section~\ref{sec:transport_partition}); this model contains no such process,
and the \SI{26}{\micro\second} renewal threshold quoted above is the map's own
measure of how fast one would have to be to matter.
...
$|\mathrm{d}\ln\Robs/\mathrm{d}\ln\bdep| < 1$; and the accuracy of the
excited-state closure, $6.73\times10^{-9}$ at the worst point. These are properties of the operator
$\Lmat(\Te,\nel)$ at a fixed operating point, or theorems about its structure.
The function $f_3(u)-f_4(u)$ says how the line ratio responds to whatever the
reservoir does; where on that function the plasma sits is the reservoir's
business, not the operator's. Transport changes the reservoir's history and
...
is built from it; the breakdown census; the sensitivity map $\overline{S}$ and
the ridge location, both evaluated at the CRE operating point
$u^{\mathrm{CRE}}$ that transport can move (Section~\ref{sec:ridge_conditional});
and every time-averaged magnitude in Chapter~\ref{ch:results}. If recycling renews $\ngs$ on a timescale shorter than
...
condition is measured to fail at the points that matter, with
$\tauesc/\tauslow$ between $2.5\times10^{-4}$ and $0.64$ over the $45$ pairs at
both scale lengths (Table~\ref{tab:transport_selection}).

\subsection{The claim, restated as the conditional it is}

The correct form is therefore not a caveat but a conditional, and it should be
written this way wherever the number appears:

\begin{quote}
\emph{If} the reservoir history $u(t)$ follows the local zero-dimensional
collisional--radiative dynamics used here, with no external influx, the error
is as computed; during the fast-stage plateau specifically, $\ngs$ is frozen to
within the closure residual.
\end{quote}

The frozen-reservoir part is exact for the plateau, because the excited manifold
equilibrates in nanoseconds while the reservoir has not yet moved, and the whole
conditional is the standard implicit assumption of every
...
are measured to differ by a ratio $\tauesc/\tauslow$ between $2.5\times10^{-4}$ and $0.64$ over the
defended breakdown pairs and by $3.2\times10^{3}$ at the cold worst point
(Section~\ref{sec:limits_summary}), and are assumed not to interact.
...
\subsection{One chord, one point: the line-of-sight integration}
\label{sec:los_limitation}

The same absence of a spatial dimension reaches the observable itself. Every
ratio in this thesis is formed from populations at a single $(\Te,\nel)$,
whereas a spectrometer forms its ratio from two brightnesses, each integrated
along a chord that crosses whatever gradients the divertor has. As
Section~\ref{sec:spectrometer} sets out, the measured ratio is the
$n_4$-weighted average of the local ratio along that chord, and it equals the
local ratio only where the local ratio is constant over the emitting region.
The weighting is by $H_\beta$ emissivity and not by path length, so the
brightest stretch of the chord sets the answer and the cold, dim stretch
contributes almost nothing.

What the calculation here establishes is therefore the error a table carries at
a point. Converting it into the error a real chord carries would require the
emissivity profile along the sightline, which is a transport result and not an
atomic one; the two-region or edge-code coupling named in
Section~\ref{sec:transport_partition} is what would supply it. The local
statement is the one this thesis defends. The chord statement is conditional on
$n_3/n_4$ being uniform over the emitting volume, and nothing here tests that.
...
\subsection{An atomic-only state space}
...
\subsection{Molecular activated recombination as a third feed stream}
...
offers no direct calculation of how much. A sensitivity scenario can
nonetheless be constructed by imposing the emissivity fractions cited in this
section. For a given
...
$\ffrac{p} \to \ffrac{p}(1-\phi_p)$. Taking $\phi_3 = 0.60$ from the reported
\SIrange{60}{70}{\percent} of $\mathrm{D}\alpha$, and $\phi_4 = 0.30$ as a
chosen value, since the sources report no $n=4$ fraction, the $\mathrm{D}\gamma$
\SIrange{10}{20}{\percent} belongs to $n=5$, and an $n=3$ source alone at
$\phi_3 = 0.60$ already puts $0.216$ into $n=4$ on this operator
(\texttt{validation/molecular\_channel/}), the difference of ground-fed
...
The scenario is crude, it inherits every uncertainty in the quoted fractions,
it applies only where those fractions do, and it holds the transient fixed: a
molecular reservoir would also move $u(t)$ and $\nion(t)$, which an imposed
source cannot.

The \emph{structural} results, the list of Section~\ref{sec:transport_partition},
are theorems about whatever matrix $\Lmat$ is supplied, and they hold for one
containing molecular states fed from the same two reservoirs, which changes the
numerical value of the ingredients and not the form of the result. They do not
hold for molecular hydrogen as a third independent reservoir: the observable
then depends on two reservoir coordinates, $u$ and $n_{\mathrm{H}_2}/\nion$, the
logistic form in $u$ survives only at a fixed value of the second, and the error
depends on two histories rather than one (Section~\ref{sec:further_work}).
...
atomic model, its cold-edge numbers are atomic-model numbers, and the scenario
...
\subsection{Mechanism and location as separate claims}
...
\subsection{The neutral density as the controlling variable}
...
\subsection{Ridge location under a rescaled neutral density}
...
$\Te = \SI{2.02}{\electronvolt}$ (\texttt{verify\_ng\_scaling.py},
\texttt{validation/ng\_scaling/}):
...
\begin{tabular}{lr}
  ...
  $\times 0.1$  & $2.30\times10^{12}\,\si{\per\cubic\centi\metre}$ \\
  $\times 1$ (the model's own CRE value) & $1.70\times10^{13}\,\si{\per\cubic\centi\metre}$ \\
  $\times 10$   & $2.50\times10^{14}\,\si{\per\cubic\centi\metre}$ \\
...
by a two-decade change in $\ngs$. The values in the table are those of the
stamped scan, the operator at $[15,j]$ with its own CRE $\ngs$ scaled.
...
\subsection{The verbal match between the ridge and detachment}
...
\subsection{The ridge density against the detached-divertor band}
...
$3.1$ times below the top, and the precise statement is the range rather than
...
\subsection{A structure without a named tokamak region}

The resulting statement is uncomfortable and should be made anyway. \textbf{The
structure this work found is real and reproducible, and it sits at a density
this thesis cannot identify with any named region of a tokamak.} The model does
not identify it with detachment or with the upstream separatrix, and calling it
either would be a claim about edge transport that a $0$-D atomic model cannot
support. The ridge
...
(Table~\ref{tab:trapping_census}), so within the closed-reservoir atomic model,
at the densities where the diagnostic is most used, no tested case reaches the
threshold.

%-----------------------------------------------------------------------------
\section{The open ion boundary: a closed-parcel test}
...
\subsection{The fixed ion density in the rate equation}
...
\subsection{Closing the manifold with a 44th ion state}
...
$10^{3}$. The objection was survived everywhere it was pressed.
...
\subsection{One population coefficient agrees and one does not}
...
\subsection{The recombination-fed and ground-fed coefficients $r_0$ and $r_1$}
...
This model resolves $\ell$ below $n=9$, so the closures differ. That
difference is tempting as the explanation, since recomputing the model with
proton $\ell$-mixing removed moves $r_1(2)$ by a factor $118$ and $r_1(3)$ by a
factor $3.35$ at $\nel = 10^{12}\,\si{\per\cubic\centi\metre}$. That argument
does not survive its own test. Removing $\ell$-mixing is the limit in which the process is
...
thesis does not explain it. Nor does the deficit stop short of
$\ffrac{3}-\ffrac{4}$ on the ground that the difference never passes through a
bundled quantity; bundling is not the protection.
...
\subsection{Truncation at $n_{\max} = 15$ and bundling above $n = 8$}
...
a way out, and the de-excitation and cascade into it from those levels; the
missing loss is upward excitation, not cascade, which runs downward. Against
published values
...
\subsection{Collisional $\ell$-mixing against escape in the bundled shells}
...
here and left in place. What survives is narrower than the radiative
criterion suggests, and it rests on a different mechanism. Where mixing wins,
the $\ell$-distribution is statistical whatever the feeds do. Where collisional
transfer wins, at the two highest densities below \SI{3}{\electronvolt}, the
distribution is set by the feeds. Adjacent-shell $n$-changing collisions, which
carry \SIrange{71}{95}{\percent} of the collisional loss, might be supposed
close to $\ell$-blind at high $n$. They are not. In the CCC rates the model is built from, which
resolve $\ell$ on both sides up to $n = 10$, the adjacent upward transfer out
of $(n,\ell)$ varies across $\ell$ by a factor that grows with $n$, from $1.22$
at $n = 2$ to between $2.5$ and $2.8$ at $n = 9$, and the downward transfer by
a factor of $5$ to $6$ at $n = 7$ to $10$, largest at the highest $\ell$ in
every case; weighted by $2\ell+1$ the scatter at $n = 9$ is
\SIrange{27}{46}{\percent}. The transfer into a shell from a statistically
populated neighbour is not statistical either: its departure coefficient runs
from $0.28$ at $\ell' = 0$ to $1.66$ at $\ell' = n-1$ for $n = 9$ and $10$.
What licenses the statistical distribution at those $121$ combinations is
detailed balance, not blindness. Every $\ell$-resolved rate obeys
$K_{\mathrm{deexc}} = K_{\mathrm{exc}}\,(\omega_i/\omega_f)\,
\mathrm{e}^{\Delta E/k\Te}$, so when the two neighbouring shells stand in their
mutual Boltzmann ratio the statistical distribution is the fixed point of the
collisional exchange whatever the $\ell$-dependence of the rates: the
departure of sublevel $\ell$ is $\theta - 1$ times a factor bounded by
$0.16$, with $\theta$ the Boltzmann departure of the neighbour pair. At the
$34$ grid points the $121$ combinations occupy, the model's own populations
give $|\theta - 1| \le 6\times10^{-5}$ for every bundled pair, and every
shell from $n = 8$ to $15$ sits within $10^{-4}$ of Saha--Boltzmann
equilibrium with the continuum: the high shells are thermalised there. The
model's own $n = 8$, its highest $\ell$-resolved shell, computed with the
$\ell$-mixing block removed, is statistical to $10^{-4}$ at every one of those
points. The two criteria are complementary: $|\theta - 1|$ exceeds $0.01$ only
below \SI{3e12}{\per\cubic\centi\metre}, where the locally evaluated mixing
exceeds the total loss in every bundled shell by at least a factor of $341$,
and no grid point fails both (\texttt{verify\_l\_blindness.py},
\texttt{validation/l\_blindness/}). The stake was not small. Had the bundled
shells been non-statistical at those points, setting their radiative decay to
zero would move $u_{\mathrm{CRE}}$ by up to \SI{1.9}{\percent} and $f_3 - f_4$
by \SI{1.2}{\percent}, and raising each shell's radiative rate to that of its
$np$ sublevel would move them by \SI{21}{\percent} and \SI{13}{\percent},
because the bundled radiative flux is $0.27$ to $0.73$ of the small net flow
into the ground state there. None of this closes the terminal-shell
over-population above, which is a different defect with a different cause,
and none of it makes the $\ell$-distribution inside $n = 11$ to $15$ a
computed quantity, since only $\ell$-summed rates exist there; for those
shells the statement rests on the Boltzmann and Saha departures alone.
...
Both limit the transfer of the magnitudes to a divertor, not the two-channel
structure, which holds for any rate set.
...
therefore safe by one to two orders of magnitude at the median of that set, not
by three. The governing quantity is in any case
...
ratio must never be quoted for or against this result. The margin is not
uniform across the breakdown set, and the bottom of the range is worth stating
plainly: evaluating the same lag law at $D_{\rm e} = 1.95$ gives $0.78$ of the
step plateau, a suppression of \SI{22}{\percent}, which is the size of the
errors this chapter is discussing. At the median $D_{\rm e} = 11.2$ it is
\SI{4}{\percent} and at $D_{\rm e} = 288$ it is \SI{0.2}{\percent}. Those three
figures are arithmetic on the stamped ratios and the law above, not a separate
run. Where the step idealisation matters most it is therefore good to a few per
cent, and at the single least favourable pair it is not.
...
rather than left scattered, so that what this thesis does not determine is
visible at once. Each is stated with the reason it is out of reach and with
...
$\Lmat\nvec + \Svec\nion = 0$; in a divertor it is set by recycling and
transport, on a timescale shorter than the local ionisation balance with
$\tauesc/\tauslow$ between $2.5\times10^{-4}$ and $0.64$ over the $45$ defended
breakdown pairs, median $0.0079$ at $L = \SI{10}{\centi\metre}$, and by $3.2\times10^{3}$ at the
cold worst point, while at the benchmark transport is the slower of the two, at
$3.6$ (Section~\ref{sec:transport}). The open-boundary test of
...
range $\tauesc/\tauslow$ runs from $2.5\times10^{-4}$ to $0.64$ over the $45$
defended breakdown pairs, with a median $\tauesc/\tauslow$ of
...
divertor band, and cannot be assigned to either by this model.
...
$\Te \ge \SI{2}{\electronvolt}$, where trapping is now shown not to move the
defended census within the tested escape-factor model ($D = 1$ to
\SI{20}{\centi\metre}, $\Tn = \Te$), so the agreement with Griem's boundary
criterion is not an artifact of neglected opacity. At citable divertor densities above \SI{2}{\electronvolt} the
\SI{100}{\micro\second} time-averaged error never reaches \SI{10}{\percent} over
$108$ pairs, worst case \SI{7.2}{\percent} (Section~\ref{sec:not_detachment}).

\subsection{The boundary of the claim}

The pattern is consistent, and it is why this chapter is not an apology. Every
objection that could be computed inside a $0$-D atomic model was computed. The
structural result and its bound came through unchanged; the census numbers came
through as conditional magnitudes, unchanged under Lyman trapping and under the
closed boundary, and shown by Section~\ref{sec:transport_selection} to rest on a
reservoir history the model does not supply at the points that carry them. The
