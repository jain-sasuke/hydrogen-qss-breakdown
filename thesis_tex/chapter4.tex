...
a measurement and not a blind instrument. What the section does establish is that the
disagreement is not evidence of an error in the quantity Chapter~\ref{ch:results}
uses. Nothing in that chapter passes through a bundled coefficient:
$\ffrac{3} - \ffrac{4}$ is a difference of two responses within one
$\ell$-resolved operator, eleven shells below the truncation and five below the
bundling boundary. An unexplained disagreement in an absolute population
coefficient is reported here as unexplained.
...
\SI{9.3}{\percent} at the benchmark, \SI{5.7}{\percent} at the grid median, and
within a factor of two at every point. An
...
\SI{9.3}{\percent} at the benchmark and to a median \SI{5.7}{\percent} across the
...
Excitation rates against RMPS (Ch.~\ref{ch:atoms}) & severe & stamped & \SI{82.1}{\percent} within \SI{20}{\percent} for $n_{\rm upper}\le4$; $n=5$ disagrees and the direction is not established \\
...
ACD96 against this model's recombination coefficient & external, passes & stamped & all $400$ points within a factor two; $\eta$ from $0.729$ to $1.042$, median $0.943$, \SI{9.3}{\percent} at the benchmark \\
...
Gate~D, SCD96 against ADAS & \textbf{FAIL} & stamped & $\eta \in [5.73,\ 8346]$, $0$ of $400$ within a factor $2$. Rebuilt on the ground-fed channel alone it reaches $323$ of $400$; the gate itself is unchanged \\
\bottomrule
\end{tabular}
\end{table}
