...
\begin{table}[tbp]
  ...
  $2.7\times10^{-10}$. An earlier version of this table integrated on a uniform
  \SI{25}{\nano\second} mesh, which under-resolves the nanosecond rise and read
  low by $6\times10^{-5}$ to $5\times10^{-4}$ relative. The estimate reproduces
...
The estimate is accurate for this observable and not for that one, and the
spectrum says which is which; why it is nevertheless not a bound, the
denominator of the ratio relaxing on the same $\tauslow$, is set out in
