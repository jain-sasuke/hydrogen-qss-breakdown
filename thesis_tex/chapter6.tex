...
\SI{2}{\electronvolt} whose ELM-averaged lower bound exceeds \SI{10}{\percent}
...
census because $\tauslow$ is long, since the ELM-averaged bound of
...
$\partial\ln u^{\mathrm{CRE}}/\partial\ln\Te$ and therefore assumes the ground
state responds through collisional--radiative balance alone; $\epsplat$, which
is built from it; the breakdown census; and every ELM-averaged magnitude in
Chapter~\ref{ch:results}. If recycling renews $\ngs$ on a timescale shorter than
$\tauslow$, $G$ is not the gain the plasma has, and these numbers are upper
estimates of an error whose true size this model cannot compute.
...
\todo{Molecular channels are not in $\Lmat$. The dilution bound above is
constructed from published emissivity fractions rather than computed here, and
should be replaced by a calculation if molecular states are ever added.}
...
the primary source once its full text is available.}
