...
fraction should appear in the line it prints.
...
Chapter~\ref{ch:theory}. $\tauqss$ is
...
sends $\Delta\to-\Delta$ and leaves both $|\Delta|$ and the logarithmic ratio
unchanged, so the gate cannot see it. Swapping $c_3\leftrightarrow c_4$ also
...
\todo{Commit the script that produces the grid-wide condition number of
$\Lmat_{FF}$, then delete this bracket.}
...
table's density rows run at $\log_{10}\nel = 12, 14, 16, 17, 18$, non-uniformly
...
gives ratios in the range $0.817$ to $0.977$ across $p = 2$ to $7$: flat to
within \SI{10}{\percent}, against a factor-$50$ spread as published.
\end{enumerate}
...
\todo{Re-read Fujimoto Table 4.1(b) directly, with the $\log_{10}\nel$ row labels
