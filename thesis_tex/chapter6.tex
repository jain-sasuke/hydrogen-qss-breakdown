...
worst lower bound over $108$ points is \SI{7.2}{\percent}. That claim is now known to be
...
runs on. That disagreement is explained rather than outstanding: the
tabulation is bundled in $n$ and this model resolves $\ell$, and the model's
own $r_1(2)$ moves by a factor $118$ when the proton $\ell$-mixing is switched
off, so the comparison measures the $\ell$~closure and cannot adjudicate a
factor of $8$ (Section~\ref{sec:r1_deficit}). Explaining a disagreement is not
the same as passing a test, and no $\ell$-resolved tabulation was available to
put in its place.
