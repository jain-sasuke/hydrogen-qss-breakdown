...
%     versions carry the EVIDENTIAL standing. The sec:r1_deficit contradiction
%     is now CLOSED: backlog I.1 (10 Sep 2026) and findings_10 ADDENDUM I
%     settle the table as bundled-n, and the section title, its 6.10 summary,
%     the Ch.4 ladder row and its caption, and the sec:further_work item have
%     all been brought into line. The r_1 deficit is reported as an l-closure
%     difference, not a model defect. Do not reintroduce the self-indictment.
%
...
\paragraph{The point the table belongs to, recovered.} An earlier draft
carried Table~\ref{tab:faultinject} without recording which grid point the
residuals were evaluated at, which is a real defect: a residual is a property of
the model \emph{at a point}, and three of the four $\max|\Delta\Lmat|$ values
scale with $\nel$, so without the point the table cannot be reproduced. Rather
than re-run somewhere convenient and relabel it, the point was recovered.
Sweeping all $400$ grid points and matching on those three values identifies
$[23,5]$, the benchmark, with a worst disagreement of \SI{1.63}{\percent}, which
is the rounding of the printed table to two significant figures. At that point
every residual in Table~\ref{tab:faultinject} reproduces to the digits shown
(\texttt{verify\_fault\_injection.py}, stamped in
\texttt{validation/fault\_injection/}).

The script rebuilds $\Lmat$ through the pipeline's own assembly rather than
patching the stored matrix, and it checks that the rebuild reproduces
\texttt{L\_grid} before injecting anything: the relative maximum difference is
exactly zero. That check is what makes the injected faults faults in the model
rather than in a re-implementation of it.
...
\begin{table}[tbp]
  ...
  one measures the $\ell$~closure rather than this model, and one fails.}
  \label{tab:ladder}
  \small
  \begin{tabular}{p{4.3cm}p{2.0cm}p{5.6cm}}
    ...
    Fujimoto Table~4.1(b), $r_1$ & external, tests the $\ell$~closure & $8.3\times$ low at $p=3$; transcription verified against the printed table, which is bundled in $n$ (\S\ref{sec:fujimoto}) \\
