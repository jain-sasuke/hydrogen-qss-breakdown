...
FOUR TERMS IN L:
    ...
    5. Proton-impact ℓ-mixing (K_lmix, n=2–8 resolved block only):
       L[i,j] += K_lmix[i,j] * ne   (gain to dest. sublevel i, same-n)
       diagonal: -= sum of outgoing ℓ-mixing rates * ne
       Conservative: column sums unchanged (no ionisation loss added).
       Computed by compute_lmix.py using PSM20 Debye formula.
       Assumes T_i = T_e and n_p = n_e (quasi-neutrality).
       Bundled n=9–15 shells use statistical ℓ-equilibrium (not in L).
...
PATHS = {
    ...
    # Term 5: proton-impact ℓ-mixing (n=2–8 resolved block, Δℓ=±1)
    # Generated by compute_lmix.py using PSM20 Debye formula
    # Bundled n=9–15 use statistical ℓ-equilibrium (not in this table)
    'K_lmix':        'data/processed/lmix/K_lmix.npy',
}
...
def build_L(Te_idx, ne, rates):
    ...
    FIVE TERMS:
      1. Collisional excitation/de-excitation  (CCC + V&S)
      2. Radiative decay                       (Hoang Binh A-coefficients)
      3. Ionisation                            (CCC TICS)
      4. Recombination → SOURCE VECTOR only    (not in L)
      5. Proton-impact ℓ-mixing               (PSM20, n=2–8, Δℓ=±1)
    """
    L = np.zeros((43, 43))
    ...
    # ── 5. Proton-impact ℓ-mixing ─────────────────────────────────────────────
    # Conservative intra-shell redistribution: column sums unchanged.
    # K_lmix[i,j,Te_idx] * ne = rate j→i [s^-1]  (non-zero only for
    # same-n, Δℓ=±1 pairs in n=2–8 resolved block).
    # n_p = n_e  (quasi-neutrality), T_i = T_e  (phase-1 approximation).
    Lm_off = rates['K_lmix'][:, :, Te_idx] * ne   # (43,43)
    np.fill_diagonal(Lm_off, 0.0)                  # zero diagonal first
    L      += Lm_off                               # off-diagonal gains
    np.fill_diagonal(L, np.diag(L) - Lm_off.sum(axis=0))  # diagonal losses
