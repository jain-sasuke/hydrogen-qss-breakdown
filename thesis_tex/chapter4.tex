...
Chapter~\ref{ch:theory}. $\tauqss$ is boundary-dependent, which
...
(\texttt{thesis\_ready.md} A1), so $\tauqss$ is the collisional--radiative
...
(10 September 2026), an uncommitted direct computation from the canonical
matrix, which agrees with the range asserted at \texttt{chapter3.tex}:719--721
to \SI{0.1}{\percent} and \SI{0.2}{\percent}. Two computations agreeing is
better than one assertion standing alone, and it is still weaker than a
committed script, which is what this needs. \texttt{np.linalg.cond} appears at one
site in \texttt{src/}, inside \texttt{verify\_fujimoto\_table41.py}:334, and
covers four point-variants at $\Te = \SI{10}{\electronvolt}$ only. The value is
corroborated and not yet reproducible from the repository.
...
fraction of $\taudrive$ over which an exponential decay on $\tauqss$ keeps it
...
faster than $\tauqss$, because there is no intermediate mode for it to decay
...
\item \textbf{A one-decade density offset reconciles the whole column.}
Comparing this model at $\SI{e13}{\per\cubic\centi\metre}$ instead of
$\SI{e12}{\per\cubic\centi\metre}$ gives ratios in the range $0.817$ to
$0.977$ across $p = 2$ to $7$: flat to within \SI{10}{\percent}, against a
factor-$50$ spread as published. A single offset removing a factor-$50$
structure is unlikely to be a coincidence. What it is remains open: the table's
density rows are recorded as running at $\log_{10}\nel = 12, 14, 16, 17, 18$,
non-uniformly spaced, and a simple off-by-one read of \emph{that} list would
land on $10^{14}$, not $10^{13}$. So the offset that works is not the offset the
recorded row labels would produce, and reading the printed table is the only
way to settle which is right.
\end{enumerate}
...
density-row labels. Until that is done, no conclusion in this thesis rests on
it. The $r_0$ agreement above is not insulated either: it was read from the same
rows, so if the row labels are wrong it inherits the same doubt. What protects
$r_0$ is that its agreement is close over a range of $p$ and at several
densities, which a systematic row offset would be unlikely to produce; that is
weaker than an independent confirmation and should not be described as one.
...
regeneration together establish that the linear algebra does what
Chapter~\ref{ch:theory} says it does, to a precision many orders of magnitude
finer than any quantity the thesis reports. The conditioning measurements point
the same way and are not counted here, because
Section~\ref{sec:conditioning} marks the grid-wide condition number as having no
committed producing script.
